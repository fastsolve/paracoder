/*
 * ccs/codegen/lib/ccs_solve_utriut/ccs_solve_utriut_mex.c
 *
 * Auxiliary code for mexFunction of ccs_solve_utriut
 *
 * C source code generated by m2c.
 * %#m2c options:8e9760455ebfcbcb3e98754644bf9e1b
 *
 */
...
static void __ccs_solve_utriut_api(mxArray **plhs, const mxArray ** prhs) {
    CCS_Matrix           U;
    emxArray_real_T      b;

    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("ccs_solve_utriut:WrongInputType",
            "Input argument U has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("ccs_solve_utriut:WrongSizeOfInputArg",
...
static void __ccs_solve_utril_3args_api(mxArray **plhs, const mxArray ** prhs) {
    CCS_Matrix           L;
    emxArray_real_T      b;
    int32_T              offset;
    ...
    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("ccs_solve_utril_3args:WrongInputType",
            "Input argument L has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("ccs_solve_utril_3args:WrongSizeOfInputArg",
    ...
    /* Invoke the target function */
    ccs_solve_utril_3args(&L, &b, offset);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 2) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("ccs_solve_utriut:TooManyOutputArguments",
                "Too many output arguments for entry-point ccs_solve_utriut.\n");
        /* Call the API function. */
        __ccs_solve_utriut_api(plhs, prhs);
    }
    else if (nrhs == 3) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("ccs_solve_utril_3args:TooManyOutputArguments",
                "Too many output arguments for entry-point ccs_solve_utril_3args.\n");
        /* Call the API function. */
        __ccs_solve_utril_3args_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("ccs_solve_utriut:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point ccs_solve_utriut.");
}
