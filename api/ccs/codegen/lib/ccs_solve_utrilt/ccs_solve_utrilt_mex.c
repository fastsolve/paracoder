/*
 * ccs/codegen/lib/ccs_solve_utrilt/ccs_solve_utrilt_mex.c
 *
 * Auxiliary code for mexFunction of ccs_solve_utrilt
 *
 * C source code generated by m2c.
 * %#m2c options:99ce410b99bab8e56046bb55c6faac7c
 *
 */
...
static void __ccs_solve_utrilt_api(mxArray **plhs, const mxArray ** prhs) {
    CCS_Matrix           L;
    emxArray_real_T      b;

    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("ccs_solve_utrilt:WrongInputType",
            "Input argument L has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("ccs_solve_utrilt:WrongSizeOfInputArg",
...
static void __ccs_solve_utrilt_3args_api(mxArray **plhs, const mxArray ** prhs) {
    CCS_Matrix           U;
    emxArray_real_T      b;
    int32_T              offset;
    ...
    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("ccs_solve_utrilt_3args:WrongInputType",
            "Input argument U has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("ccs_solve_utrilt_3args:WrongSizeOfInputArg",
            "Argument U should be a scalar.");
    marshallin_const_CCS_Matrix(&U, prhs[0], "U");
    ...
    /* Invoke the target function */
    ccs_solve_utrilt_3args(&U, &b, offset);

    /* Deallocate input and marshall out function outputs */
    destroy_CCS_Matrix(&U);
    plhs[0] = move_emxArray_to_mxArray((emxArray__common*)(&b), mxDOUBLE_CLASS);
    mxFree(b.size);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 2) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("ccs_solve_utrilt:TooManyOutputArguments",
                "Too many output arguments for entry-point ccs_solve_utrilt.\n");
        /* Call the API function. */
        __ccs_solve_utrilt_api(plhs, prhs);
    }
    else if (nrhs == 3) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("ccs_solve_utrilt_3args:TooManyOutputArguments",
                "Too many output arguments for entry-point ccs_solve_utrilt_3args.\n");
        /* Call the API function. */
        __ccs_solve_utrilt_3args_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("ccs_solve_utrilt:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point ccs_solve_utrilt.");
}
