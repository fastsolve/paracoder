/*
 * ccs/codegen/lib/ccs_solve_triu/ccs_solve_triu_mex.c
 *
 * Auxiliary code for mexFunction of ccs_solve_triu
 *
...
static void __ccs_solve_triu_api(mxArray **plhs, const mxArray ** prhs) {
    CCS_Matrix           U;
    emxArray_real_T      b;

    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("ccs_solve_triu:WrongInputType",
            "Input argument U has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("ccs_solve_triu:WrongSizeOfInputArg",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 2) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("ccs_solve_triu:TooManyOutputArguments",
                "Too many output arguments for entry-point ccs_solve_triu.\n");
        /* Call the API function. */
        __ccs_solve_triu_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("ccs_solve_triu:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point ccs_solve_triu.");
}
