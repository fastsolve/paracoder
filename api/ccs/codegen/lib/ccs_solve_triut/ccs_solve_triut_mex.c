/*
 * ccs/codegen/lib/ccs_solve_triut/ccs_solve_triut_mex.c
 *
 * Auxiliary code for mexFunction of ccs_solve_triut
 *
 * C source code generated by m2c.
 * %#m2c options:a0ddb896d8d48c48f79a78942251e346
 *
 */
...
static void __ccs_solve_triut_api(mxArray **plhs, const mxArray ** prhs) {
    CCS_Matrix           U;
    emxArray_real_T      b;

    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("ccs_solve_triut:WrongInputType",
            "Input argument U has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("ccs_solve_triut:WrongSizeOfInputArg",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 2) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("ccs_solve_triut:TooManyOutputArguments",
                "Too many output arguments for entry-point ccs_solve_triut.\n");
        /* Call the API function. */
        __ccs_solve_triut_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("ccs_solve_triut:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point ccs_solve_triut.");
}
