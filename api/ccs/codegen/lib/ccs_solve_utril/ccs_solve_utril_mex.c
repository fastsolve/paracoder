/*
 * ccs/codegen/lib/ccs_solve_utril/ccs_solve_utril_mex.c
 *
 * Auxiliary code for mexFunction of ccs_solve_utril
 *
 * C source code generated by m2c.
 * %#m2c options:9f5b6a0f6589d55938b44c6f24f9c4da
 *
 */
...
static void __ccs_solve_utril_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            L;
    emxArray_real_T      b;

    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("ccs_solve_utril:WrongInputType",
            "Input argument L has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("ccs_solve_utril:WrongSizeOfInputArg",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 2) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("ccs_solve_utril:TooManyOutputArguments",
                "Too many output arguments for entry-point ccs_solve_utril.\n");
        /* Call the API function. */
        __ccs_solve_utril_api(plhs, prhs);
    }
    else if (nrhs == 3) {
        if (nlhs > 1)
        ...
        mexErrMsgIdAndTxt("ccs_solve_utril:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point ccs_solve_utril.");
}
