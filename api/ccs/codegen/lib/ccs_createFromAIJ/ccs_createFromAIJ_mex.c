/*
 * ccs/codegen/lib/ccs_createFromAIJ/ccs_createFromAIJ_mex.c
 *
 * Auxiliary code for mexFunction of ccs_createFromAIJ
 *
 * C source code generated by m2c.
 * %#m2c options:94d8d6d8a1d6e4e6ba9fa905b7689567
 *
 */
...
static mxArray *marshallout_struct0_T(struct0_T *pStruct) {
    const char           *fields[] = {"col_ptr", "row_ind", "val", "nrows", "ncols"};
    int                  one=1;
    mxArray              *mx = create_struct_mxArray(1, &one, 5, fields);

    mxSetField(mx, 0, "col_ptr", move_emxArray_to_mxArray((emxArray__common*)(pStruct->col_ptr), mxINT32_CLASS));
    mxFree(pStruct->col_ptr->size);
    mxFree(pStruct->col_ptr);

    mxSetField(mx, 0, "row_ind", move_emxArray_to_mxArray((emxArray__common*)(pStruct->row_ind), mxINT32_CLASS));
    mxFree(pStruct->row_ind->size);
...
static void __ccs_createFromAIJ_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    ccs_createFromAIJ(&rows, &cols, &vs, &A);
...
static void __ccs_create1_api(mxArray **plhs, const mxArray ** prhs) {
    emxArray_int32_T     is;
    emxArray_int32_T     js;
    emxArray_real_T      vs;
    int32_T              ni;
    ...
    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("ccs_create1:WrongInputType",
            "Input argument is has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[0]) && mxGetDimensions(prhs[0])[1] != 1) 
        mexErrMsgIdAndTxt("ccs_create1:WrongSizeOfInputArg",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 3) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("ccs_createFromAIJ:TooManyOutputArguments",
                "Too many output arguments for entry-point ccs_createFromAIJ.\n");
        /* Call the API function. */
        __ccs_createFromAIJ_api(plhs, prhs);
    }
    else if (nrhs == 5) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("ccs_create1:TooManyOutputArguments",
                "Too many output arguments for entry-point ccs_create1.\n");
        /* Call the API function. */
        __ccs_create1_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("ccs_createFromAIJ:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point ccs_createFromAIJ.");
}
