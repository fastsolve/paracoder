/*
 * ccs/codegen/lib/ccs_solve_trilt/ccs_solve_trilt_mex.c
 *
 * Auxiliary code for mexFunction of ccs_solve_trilt
 *
...
static void __ccs_solve_trilt_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            L;
    emxArray_real_T      b;

    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("ccs_solve_trilt:WrongInputType",
            "Input argument L has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("ccs_solve_trilt:WrongSizeOfInputArg",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 2) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("ccs_solve_trilt:TooManyOutputArguments",
                "Too many output arguments for entry-point ccs_solve_trilt.\n");
        /* Call the API function. */
        __ccs_solve_trilt_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("ccs_solve_trilt:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point ccs_solve_trilt.");
}
