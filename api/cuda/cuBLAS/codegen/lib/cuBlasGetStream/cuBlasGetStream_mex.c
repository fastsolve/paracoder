...
static void __cuBlasGetStream_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            cubHdl;
    struct0_T            strm;
    int32_T             *errCode;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("cuBlasGetStream:WrongSizeOfInputArg",
            "Argument cubHdl should be a scalar.");
    marshallin_const_struct0_T(&cubHdl, prhs[0], "cubHdl");
    prealloc_struct0_T(&strm);
    ...
    destroy_struct0_T(&cubHdl);
    plhs[0] = marshallout_struct0_T(&strm);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("cuBlasGetStream:TooManyOutputArguments",
                "Too many output arguments for entry-point cuBlasGetStream.\n");
        /* Call the API function. */
        __cuBlasGetStream_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("cuBlasGetStream:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point cuBlasGetStream.");
