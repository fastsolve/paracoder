#include "cuBlasSetAtomicsMode.h"
#include "m2c.h"
#include "mcuda.h"
...
static void b_m2c_error(const emxArray_char_T *varargin_3)
{
  ...
  M2C_error("cuBlas:RuntimeError",
            "cublasSetAtomicsMode returned error code %s\n", &b_varargin_3->
            data[0]);
...
void cuBlasSetAtomicsMode(const struct0_T *hdl, int mode, int *errCode,
  boolean_T *toplevel)
{
  boolean_T p;
  boolean_T b_p;
  ...
  *errCode = cublasSetAtomicsMode(c_hdl, mode);
  *toplevel = true;
...
void cuBlasSetAtomicsMode_initialize(void)
{
}

void cuBlasSetAtomicsMode_terminate(void)
{
}
