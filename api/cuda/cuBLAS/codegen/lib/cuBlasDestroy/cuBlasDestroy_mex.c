 ...
 * cuBlasDestroy_mex.c
 *
 * Auxiliary code for mexFunction of cuBlasDestroy
 *
...
static void __cuBlasDestroy_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            hdl;
    int32_T             *errCode;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("cuBlasDestroy:WrongInputType",
            "Input argument hdl has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("cuBlasDestroy:WrongSizeOfInputArg",
    ...
    cuBlasDestroy(&hdl, errCode, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("cuBlasDestroy:TooManyOutputArguments",
                "Too many output arguments for entry-point cuBlasDestroy.\n");
        /* Call the API function. */
        __cuBlasDestroy_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("cuBlasDestroy:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point cuBlasDestroy.");
