...
void cuVecDestroy(struct0_T *vec, int *errCode, boolean_T *toplevel)
{
  ...
  *errCode = cudaFree(output);
  if (*errCode != 0) {
    ...
    c_msg0->size[0] = 1;
    c_msg0->size[1] = len;
    emxEnsureCapacity((emxArray__common *)c_msg0, i1, (int)sizeof(unsigned char));
    for (i1 = 0; i1 < len; i1++) {
    ...
    i1 = d_msg0->size[0];
    d_msg0->size[0] = len;
    emxEnsureCapacity((emxArray__common *)d_msg0, i1, (int)sizeof(char));
    for (i1 = 0; i1 < len; i1++) {
    ...
    b_msg0->size[0] = 1;
    b_msg0->size[1] = len;
    emxEnsureCapacity((emxArray__common *)b_msg0, i1, (int)sizeof(char));
    for (i1 = 0; i1 < len; i1++) {
