/*
 * cuBlasSetPointerMode_mex.c
 *
 * Auxiliary code for mexFunction of cuBlasSetPointerMode
 *
...
static void __cuBlasSetPointerMode_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            hdl;
    int32_T              mode;
    int32_T             *errCode;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("cuBlasSetPointerMode:WrongInputType",
            "Input argument hdl has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("cuBlasSetPointerMode:WrongSizeOfInputArg",
    ...
    cuBlasSetPointerMode(&hdl, mode, errCode, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("cuBlasSetPointerMode:TooManyOutputArguments",
                "Too many output arguments for entry-point cuBlasSetPointerMode.\n");
        /* Call the API function. */
        __cuBlasSetPointerMode_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("cuBlasSetPointerMode:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point cuBlasSetPointerMode.");
