/*
 * cuBlasSetStream_mex.c
 *
 * Auxiliary code for mexFunction of cuBlasSetStream
 *
 * C source code generated by m2c.
 * %#m2c options:6031c0acc304d62772459fc57af2c1fc
 *
 */
...
static void __cuBlasSetStream_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            hdl;
    struct0_T            strm;
    int32_T             *errCode;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("cuBlasSetStream:WrongInputType",
            "Input argument hdl has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("cuBlasSetStream:WrongSizeOfInputArg",
    ...
    marshallin_const_struct0_T(&strm, prhs[1], "strm");
    ...
    cuBlasSetStream(&hdl, &strm, errCode, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_struct0_T(&hdl);
    destroy_struct0_T(&strm);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("cuBlasSetStream:TooManyOutputArguments",
                "Too many output arguments for entry-point cuBlasSetStream.\n");
        /* Call the API function. */
        __cuBlasSetStream_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("cuBlasSetStream:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point cuBlasSetStream.");
