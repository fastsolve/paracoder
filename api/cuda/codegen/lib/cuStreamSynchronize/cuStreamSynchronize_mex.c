 ...
 * %#m2c options:afa4a6cbe4c8f5ea7a39c40fcd4a976c
 *
 */
...
static void __cuStreamSynchronize_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            stm;
    int32_T             *errCode;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("cuStreamSynchronize:WrongInputType",
            "Input argument stm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("cuStreamSynchronize:WrongSizeOfInputArg",
            "Argument stm should be a scalar.");
    marshallin_const_struct0_T(&stm, prhs[0], "stm");
    ...
    cuStreamSynchronize(&stm, errCode, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_struct0_T(&stm);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("cuStreamSynchronize:TooManyOutputArguments",
                "Too many output arguments for entry-point cuStreamSynchronize.\n");
        /* Call the API function. */
        __cuStreamSynchronize_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("cuStreamSynchronize:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point cuStreamSynchronize.");
