...
static void __cuStreamHandleDestroy_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            stm;
    int32_T             *errCode;
    boolean_T           *toplevel;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("cuStreamHandleDestroy:TooManyOutputArguments",
                "Too many output arguments for entry-point cuStreamHandleDestroy.\n");
        /* Call the API function. */
        __cuStreamHandleDestroy_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("cuStreamHandleDestroy:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point cuStreamHandleDestroy.");
