...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 1) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("cuStreamQuery:TooManyOutputArguments",
                "Too many output arguments for entry-point cuStreamQuery.\n");
        /* Call the API function. */
        __cuStreamQuery_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("cuStreamQuery:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point cuStreamQuery.");
}
