...
static void __cuStreamHandleCreate_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            stm;
    int32_T             *errCode;
    boolean_T           *toplevel;

    /* Marshall in inputs and preallocate outputs */
    prealloc_struct0_T(&stm);
    ...
    cuStreamHandleCreate(&stm, errCode, toplevel);

    /* Deallocate input and marshall out function outputs */
    plhs[0] = marshallout_struct0_T(&stm);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 0) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("cuStreamHandleCreate:TooManyOutputArguments",
                "Too many output arguments for entry-point cuStreamHandleCreate.\n");
        /* Call the API function. */
        __cuStreamHandleCreate_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("cuStreamHandleCreate:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point cuStreamHandleCreate.");
