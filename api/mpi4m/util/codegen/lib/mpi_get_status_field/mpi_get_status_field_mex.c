/*
 * util/codegen/lib/mpi_get_status_field/mpi_get_status_field_mex.c
 *
 * Auxiliary code for mexFunction of mpi_get_status_field
 *
 * C source code generated by m2c.
 * %#m2c options:037e5ac086e5153208e4aeb4329449c7
 *
 */
...
static void __mpi_get_status_field_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       stat;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_get_status_field:WrongInputType",
            "Input argument stat has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_get_status_field:WrongSizeOfInputArg",
            "Argument stat should be a scalar.");
    marshallin_const_M2C_OpaqueType(&stat, prhs[0], "stat");

    if (mxGetNumberOfElements(prhs[1]) && mxGetClassID(prhs[1]) != mxCHAR_CLASS)
        mexErrMsgIdAndTxt("mpi_get_status_field:WrongInputType",
            "Input argument field has incorrect data type; char is expected.");
    if (mxGetNumberOfElements(prhs[1]) && mxGetDimensions(prhs[1])[0] != 1) 
        mexErrMsgIdAndTxt("mpi_get_status_field:WrongSizeOfInputArg",
            "Dimension 1 of field should be equal to 1.");
    alias_mxArray_to_emxArray(prhs[1], (emxArray__common *)(&field), "field", 2);

    /* Invoke the target function */
    _output = mpi_get_status_field(&stat, &field);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&stat);
    free_emxArray((emxArray__common*)(&field));
    plhs[0] = copy_scalar_to_mxArray(&_output, mxINT32_CLASS);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 2) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("mpi_get_status_field:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_get_status_field.\n");
        /* Call the API function. */
        __mpi_get_status_field_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_get_status_field:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_get_status_field.");
}
