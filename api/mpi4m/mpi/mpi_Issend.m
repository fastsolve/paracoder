function [req, info, toplevel] = mpi_Issend(ptr, count, datatype, dest, tag, comm)
%mpi_Issend   Starts a nonblocking synchronous send
%
%  [req, info] = mpi_Issend(ptr, count, datatype, dest, tag, comm)
%
%Input arguments:
...
%
%  SEE ALSO: mpi_Ssend, mpi_Test, mpi_Testall, mpi_Testany, mpi_Testsome,
%      mpi_Wait, mpi_Waitall, mpi_Waitany, mpi_Waitsome
%
% C interface:
%   int MPI_Issend(void *buf, int count, MPI_Datatype datatype, int dest,
%                  int tag, MPI_Comm comm, MPI_Request *request)
% http://mpi.deino.net/mpi_functions/MPI_Issend.html
...
info = coder.ceval('MPI_Issend', m2c_opaque_ptr(ptr), int32(count), ...
...
%! assert(info==0);
%! [req2,info] = mpi_Issend(m2c_opaque_ptr(var1), count, MPI_DOUBLE, int32(0), int32(1), comm);
%! assert(info==0);
%! [stat, req1, info] = mpi_Wait(req1);
%! [stat, req2, info] = mpi_Wait(req2);
%! assert(info==0);
