/*
 * mpi/codegen/lib/mpi_Cart_rank/mpi_Cart_rank_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Cart_rank
 *
...
static void __mpi_Cart_rank_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       comm;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Cart_rank:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Cart_rank:WrongSizeOfInputArg",
    ...
    if (mxGetNumberOfElements(prhs[1]) && mxGetClassID(prhs[1]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Cart_rank:WrongInputType",
            "Input argument coords has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[1]) && mxGetDimensions(prhs[1])[1] != 1) 
        mexErrMsgIdAndTxt("mpi_Cart_rank:WrongSizeOfInputArg",
            "Dimension 2 of coords should be equal to 1.");
    alias_mxArray_to_emxArray(prhs[1], (emxArray__common *)(&coords), "coords", 1);
    ...
    mpi_Cart_rank(&comm, &coords, rank, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&comm);
    free_emxArray((emxArray__common*)(&coords));
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Cart_rank:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Cart_rank.\n");
        /* Call the API function. */
        __mpi_Cart_rank_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Cart_rank:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Cart_rank.");
