...
static void b_m2c_error(const emxArray_char_T *varargin_3)
{
  ...
  M2C_error("MPI:RuntimeError",
            "MPI_Graphdims_get failed with error message %s\n",
            &b_varargin_3->data[0]);
...
void mpi_Graphdims_get(const M2C_OpaqueType *comm, int *nnodes, int *nedges,
                       int *info, boolean_T *toplevel)
{
  ...
  *info = MPI_Graphdims_get(c_comm, nnodes, nedges);
  *toplevel = true;
  if (*info != 0) {
...
void mpi_Graphdims_get_initialize(void)
{
}

void mpi_Graphdims_get_terminate(void)
{
}
