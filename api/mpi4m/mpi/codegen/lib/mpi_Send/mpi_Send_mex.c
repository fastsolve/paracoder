/*
 * mpi/codegen/lib/mpi_Send/mpi_Send_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Send
 *
...
static void __mpi_Send_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Send:WrongInputType",
            "Input argument ptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Send:WrongSizeOfInputArg",
    ...
    mpi_Send(&ptr, count, &datatype, dest, tag, &comm, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 6) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("mpi_Send:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Send.\n");
        /* Call the API function. */
        __mpi_Send_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Send:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Send.");
