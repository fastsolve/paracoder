/*
 * mpi/codegen/lib/mpi_Scan/mpi_Scan_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Scan
 *
...
static void __mpi_Scan_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Scan:WrongInputType",
            "Input argument sptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Scan:WrongSizeOfInputArg",
    ...
    mpi_Scan(&sptr, &rptr, count, &datatype, &op, &comm, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 6) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("mpi_Scan:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Scan.\n");
        /* Call the API function. */
        __mpi_Scan_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Scan:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Scan.");
