/*
 * mpi/codegen/lib/mpi_Waitall/mpi_Waitall_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Waitall
 *
...
static void __mpi_Waitall_api(mxArray **plhs, const mxArray ** prhs) {
    int32_T              count;
    M2C_OpaqueType       reqs;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Waitall:WrongInputType",
            "Input argument count has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Waitall:WrongSizeOfInputArg",
    ...
    mpi_Waitall(count, &reqs, &stats, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    /* Nothing to be done for count */
    plhs[1] = marshallout_M2C_OpaqueType(&reqs);
    plhs[0] = marshallout_M2C_OpaqueType(&stats);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 4)
            mexErrMsgIdAndTxt("mpi_Waitall:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Waitall.\n");
        /* Call the API function. */
        __mpi_Waitall_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Waitall:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Waitall.");
