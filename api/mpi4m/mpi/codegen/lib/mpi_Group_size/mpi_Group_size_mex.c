/*
 * mpi/codegen/lib/mpi_Group_size/mpi_Group_size_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Group_size
 *
...
static void __mpi_Group_size_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Group_size:WrongInputType",
            "Input argument group has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Group_size:WrongSizeOfInputArg",
    ...
    mpi_Group_size(&group, size, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Group_size:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Group_size.\n");
        /* Call the API function. */
        __mpi_Group_size_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Group_size:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Group_size.");
