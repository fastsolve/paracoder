/*
 * mpi/codegen/lib/mpi_Irsend/mpi_Irsend_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Irsend
 *
...
static void __mpi_Irsend_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Irsend:WrongInputType",
            "Input argument ptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Irsend:WrongSizeOfInputArg",
    ...
    mpi_Irsend(&ptr, count, &datatype, dest, tag, &comm, &req, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 6) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Irsend:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Irsend.\n");
        /* Call the API function. */
        __mpi_Irsend_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Irsend:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Irsend.");
