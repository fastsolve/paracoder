/*
 * mpi/codegen/lib/mpi_Comm_remote_size/mpi_Comm_remote_size_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Comm_remote_size
 *
...
static void __mpi_Comm_remote_size_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            comm;
    int32_T             *size;
    int32_T             *info;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Comm_remote_size:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Comm_remote_size:WrongSizeOfInputArg",
    ...
    mpi_Comm_remote_size(&comm, size, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Comm_remote_size:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Comm_remote_size.\n");
        /* Call the API function. */
        __mpi_Comm_remote_size_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Comm_remote_size:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Comm_remote_size.");
