#include "mpi_Comm_remote_size.h"
#include "m2c.h"
#include "mpi.h"
...
static void b_m2c_error(const emxArray_char_T *varargin_3)
{
  ...
  M2C_error("MPI:RuntimeError",
            "MPI_Comm_remote_size failed with error message %s\n",
            &b_varargin_3->data[0]);
...
void mpi_Comm_remote_size(const struct0_T *comm, int *size, int *info, boolean_T
  *toplevel)
{
  boolean_T p;
  boolean_T b_p;
  ...
  *info = MPI_Comm_remote_size(c_comm, size);
  *toplevel = true;
...
void mpi_Comm_remote_size_initialize(void)
{
}

void mpi_Comm_remote_size_terminate(void)
{
}
