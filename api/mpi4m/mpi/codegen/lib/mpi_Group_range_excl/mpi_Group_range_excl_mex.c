/*
 * mpi/codegen/lib/mpi_Group_range_excl/mpi_Group_range_excl_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Group_range_excl
 *
 * C source code generated by m2c.
 * %#m2c options:60f59d239217e50f3dd157f2f7e8f622
 *
 */
...
static void __mpi_Group_range_excl_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       group;
    int32_T              n;
    emxArray_int32_T     ranges;
    M2C_OpaqueType       newgroup;
    int32_T             *info;
    boolean_T           *toplevel;

    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Group_range_excl:WrongInputType",
            "Input argument group has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Group_range_excl:WrongSizeOfInputArg",
            "Argument group should be a scalar.");
    marshallin_const_M2C_OpaqueType(&group, prhs[0], "group");
    ...
    if (mxGetNumberOfElements(prhs[2]) && mxGetClassID(prhs[2]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Group_range_excl:WrongInputType",
            "Input argument ranges has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[2]) && mxGetDimensions(prhs[2])[1] != 1) 
        mexErrMsgIdAndTxt("mpi_Group_range_excl:WrongSizeOfInputArg",
            "Dimension 2 of ranges should be equal to 1.");
    alias_mxArray_to_emxArray(prhs[2], (emxArray__common *)(&ranges), "ranges", 1);
    prealloc_M2C_OpaqueType(&newgroup);
    ...
    mpi_Group_range_excl(&group, n, &ranges, &newgroup, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&group);
    /* Nothing to be done for n */
    free_emxArray((emxArray__common*)(&ranges));
    plhs[0] = marshallout_M2C_OpaqueType(&newgroup);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Group_range_excl:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Group_range_excl.\n");
        /* Call the API function. */
        __mpi_Group_range_excl_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Group_range_excl:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Group_range_excl.");
