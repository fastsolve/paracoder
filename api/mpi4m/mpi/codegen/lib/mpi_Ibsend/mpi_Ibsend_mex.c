/*
 * mpi/codegen/lib/mpi_Ibsend/mpi_Ibsend_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Ibsend
 *
...
static void __mpi_Ibsend_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Ibsend:WrongInputType",
            "Input argument ptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Ibsend:WrongSizeOfInputArg",
    ...
    mpi_Ibsend(&ptr, count, &datatype, dest, tag, &comm, &req, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 6) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Ibsend:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Ibsend.\n");
        /* Call the API function. */
        __mpi_Ibsend_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Ibsend:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Ibsend.");
