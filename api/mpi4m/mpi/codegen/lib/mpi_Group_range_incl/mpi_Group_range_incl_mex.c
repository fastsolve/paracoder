/*
 * mpi/codegen/lib/mpi_Group_range_incl/mpi_Group_range_incl_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Group_range_incl
 *
...
static void __mpi_Group_range_incl_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Group_range_incl:WrongInputType",
            "Input argument group has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Group_range_incl:WrongSizeOfInputArg",
    ...
    mpi_Group_range_incl(&group, n, &ranges, &newgroup, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Group_range_incl:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Group_range_incl.\n");
        /* Call the API function. */
        __mpi_Group_range_incl_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Group_range_incl:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Group_range_incl.");
