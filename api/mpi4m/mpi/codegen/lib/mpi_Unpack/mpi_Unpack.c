...
void mpi_Unpack(const M2C_OpaquePtrType *inbuf, int incount, int *position,
                const M2C_OpaquePtrType *outbuf, int packsize,
                const M2C_OpaqueType *datatype, const M2C_OpaqueType *comm,
                int *info, boolean_T *toplevel)
{
  ...
  *info =
      MPI_Unpack(ptr, incount, position, b_ptr, packsize, c_datatype, b_comm);
  *toplevel = true;
  if (*info != 0) {
