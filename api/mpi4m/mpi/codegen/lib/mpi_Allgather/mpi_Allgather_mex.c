/*
 * mpi/codegen/lib/mpi_Allgather/mpi_Allgather_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Allgather
 *
 * C source code generated by m2c.
 * %#m2c options:2ae2287fba0082e417800861aea29ed0
 *
 */
...
static void __mpi_Allgather_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Allgather:WrongInputType",
            "Input argument sptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Allgather:WrongSizeOfInputArg",
    ...
    mpi_Allgather(&sptr, scount, &stype, &rptr, rcount, &rtype, &comm, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 7) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("mpi_Allgather:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Allgather.\n");
        /* Call the API function. */
        __mpi_Allgather_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Allgather:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Allgather.");
