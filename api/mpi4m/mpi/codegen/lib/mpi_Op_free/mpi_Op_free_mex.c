/*
 * mpi/codegen/lib/mpi_Op_free/mpi_Op_free_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Op_free
 *
 * C source code generated by m2c.
 * %#m2c options:220201300d186f016d3c0ff6b0a603ce
 *
 */
...
static void __mpi_Op_free_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       op;
    int32_T             *info;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Op_free:WrongInputType",
            "Input argument op has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Op_free:WrongSizeOfInputArg",
            "Argument op should be a scalar.");
    marshallin_M2C_OpaqueType(&op, prhs[0], "op");
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Op_free:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Op_free.\n");
        /* Call the API function. */
        __mpi_Op_free_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Op_free:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Op_free.");
