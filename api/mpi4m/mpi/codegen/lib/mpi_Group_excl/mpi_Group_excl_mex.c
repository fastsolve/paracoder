/*
 * mpi/codegen/lib/mpi_Group_excl/mpi_Group_excl_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Group_excl
 *
...
static void __mpi_Group_excl_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Group_excl:WrongInputType",
            "Input argument group has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Group_excl:WrongSizeOfInputArg",
    ...
    mpi_Group_excl(&group, n, &ranks, &newgroup, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Group_excl:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Group_excl.\n");
        /* Call the API function. */
        __mpi_Group_excl_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Group_excl:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Group_excl.");
