/*
 * mpi/codegen/lib/mpi_Waitsome/mpi_Waitsome_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Waitsome
 *
...
static void __mpi_Waitsome_api(mxArray **plhs, const mxArray ** prhs) {
    int32_T              count;
    struct0_T            reqs;
    emxArray_int32_T     idxs;
    struct0_T            stats;
    int32_T             *info;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Waitsome:WrongInputType",
            "Input argument count has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Waitsome:WrongSizeOfInputArg",
    ...
    init_emxArray((emxArray__common*)(&idxs), 1);
    ...
    mpi_Waitsome(count, &reqs, &idxs, &stats, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    /* Nothing to be done for count */
    plhs[2] = marshallout_struct0_T(&reqs);
    plhs[0] = move_emxArray_to_mxArray((emxArray__common*)(&idxs), mxINT32_CLASS);
    mxFree(idxs.size);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 5)
            mexErrMsgIdAndTxt("mpi_Waitsome:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Waitsome.\n");
        /* Call the API function. */
        __mpi_Waitsome_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Waitsome:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Waitsome.");
