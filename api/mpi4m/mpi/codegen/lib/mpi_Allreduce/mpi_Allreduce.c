...
void mpi_Allreduce(const M2C_OpaquePtrType *sptr, const M2C_OpaquePtrType *rptr,
                   int count, const M2C_OpaqueType *datatype, const
                   ...
                   boolean_T *toplevel)
{
  ...
  MPI_Op obj;
  static const char b_cv[6] = { 'M', 'P', 'I', '_', 'O', 'p' };
  ...
  if (p && (op->type->size[1] != 0)) {
    ...
    while ((!exitg1) && (resultlen < 6)) {
      if (!(op->type->data[resultlen] == b_cv[resultlen])) {
        p = false;
        exitg1 = true;
      } else {
        resultlen++;
