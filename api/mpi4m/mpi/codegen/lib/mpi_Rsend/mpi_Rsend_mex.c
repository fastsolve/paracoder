/*
 * mpi/codegen/lib/mpi_Rsend/mpi_Rsend_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Rsend
 *
...
static void __mpi_Rsend_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Rsend:WrongInputType",
            "Input argument ptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Rsend:WrongSizeOfInputArg",
    ...
    mpi_Rsend(&ptr, count, &datatype, dest, tag, &comm, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 6) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("mpi_Rsend:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Rsend.\n");
        /* Call the API function. */
        __mpi_Rsend_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Rsend:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Rsend.");
