  ...
  boolean_T *toplevel)
{
  ...
  if (!p) {
    ...
    emxEnsureCapacity_char_T(b_indices, resultlen);
  ...
  if (!p) {
    ...
    for (resultlen = 0; resultlen < loop_ub; resultlen++) {
      b_indices->data[b_indices->size[0] * resultlen] = oldtypes->type->
        data[oldtypes->type->size[0] * resultlen];
    }

    b_indices->data[b_indices->size[0] * oldtypes->type->size[1]] = '\x00';
  ...
  emxEnsureCapacity_uint8_T(oldtypes0, resultlen);
  ...
  *info = MPI_Type_create_struct(count, &blocklens->data[0], ptr, b_ptr,
  ...
  if (*info != 0) {
    ...
    resultlen = b_indices->size[0] * b_indices->size[1];
    b_indices->size[0] = 1;
    b_indices->size[1] = (short)loop_ub;
    emxEnsureCapacity_char_T(b_indices, resultlen);
    loop_ub = (short)loop_ub;
    for (resultlen = 0; resultlen < loop_ub; resultlen++) {
      b_indices->data[b_indices->size[0] * resultlen] = c_msg0->data[resultlen];
    }
