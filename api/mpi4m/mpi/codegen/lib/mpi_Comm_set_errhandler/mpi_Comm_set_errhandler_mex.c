/*
 * mpi/codegen/lib/mpi_Comm_set_errhandler/mpi_Comm_set_errhandler_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Comm_set_errhandler
 *
 * C source code generated by m2c.
 * %#m2c options:f85557a9bca22d92b97f3c2e5e821a9f
 *
 */
...
static void __mpi_Comm_set_errhandler_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       comm;
    M2C_OpaqueType       errhandler;
    int32_T             *info;
    boolean_T           *toplevel;

    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Comm_set_errhandler:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Comm_set_errhandler:WrongSizeOfInputArg",
    ...
    if (mxGetNumberOfElements(prhs[1]) && mxGetClassID(prhs[1]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Comm_set_errhandler:WrongInputType",
            "Input argument errhandler has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[1]) != 1)
        mexErrMsgIdAndTxt("mpi_Comm_set_errhandler:WrongSizeOfInputArg",
            "Argument errhandler should be a scalar.");
    marshallin_const_M2C_OpaqueType(&errhandler, prhs[1], "errhandler");
    ...
    mpi_Comm_set_errhandler(&comm, &errhandler, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&comm);
    destroy_M2C_OpaqueType(&errhandler);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("mpi_Comm_set_errhandler:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Comm_set_errhandler.\n");
        /* Call the API function. */
        __mpi_Comm_set_errhandler_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Comm_set_errhandler:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Comm_set_errhandler.");
