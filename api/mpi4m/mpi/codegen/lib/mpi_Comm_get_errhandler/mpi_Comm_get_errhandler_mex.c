/*
 * mpi/codegen/lib/mpi_Comm_get_errhandler/mpi_Comm_get_errhandler_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Comm_get_errhandler
 *
...
static void __mpi_Comm_get_errhandler_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       comm;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Comm_get_errhandler:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Comm_get_errhandler:WrongSizeOfInputArg",
            "Argument comm should be a scalar.");
    marshallin_const_M2C_OpaqueType(&comm, prhs[0], "comm");
    prealloc_M2C_OpaqueType(&errhandler);
    ...
    mpi_Comm_get_errhandler(&comm, &errhandler, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&comm);
    plhs[0] = marshallout_M2C_OpaqueType(&errhandler);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Comm_get_errhandler:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Comm_get_errhandler.\n");
        /* Call the API function. */
        __mpi_Comm_get_errhandler_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Comm_get_errhandler:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Comm_get_errhandler.");
