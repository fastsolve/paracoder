                   ...
                   boolean_T *toplevel)
{
  ...
  if (!p) {
    ...
    emxEnsureCapacity((emxArray__common *)b_func, resultlen, sizeof(char));
  ...
  *info = MPI_Op_create(t_func, commute, &t_op);
  ...
  emxEnsureCapacity((emxArray__common *)op->data, resultlen, sizeof(unsigned
    char));
  ...
  op->type->size[0] = 1;
  op->type->size[1] = 11;
  emxEnsureCapacity((emxArray__common *)op->type, resultlen, sizeof(char));
  for (resultlen = 0; resultlen < 11; resultlen++) {
