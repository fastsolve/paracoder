/*
 * mpi/codegen/lib/mpi_Comm_dup/mpi_Comm_dup_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Comm_dup
 *
...
static void __mpi_Comm_dup_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       comm;
    M2C_OpaqueType       dup;
    int32_T             *info;
    boolean_T           *toplevel;
    ...
    mpi_Comm_dup(&comm, &dup, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&comm);
    plhs[0] = marshallout_M2C_OpaqueType(&dup);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Comm_dup:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Comm_dup.\n");
        /* Call the API function. */
        __mpi_Comm_dup_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Comm_dup:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Comm_dup.");
