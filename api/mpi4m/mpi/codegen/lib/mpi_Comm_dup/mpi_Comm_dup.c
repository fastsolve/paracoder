...
void mpi_Comm_dup(const M2C_OpaqueType *comm, M2C_OpaqueType *dup, int *info,
                  boolean_T *toplevel)
{
  boolean_T p;
  int sizepe;
  ...
  *info = MPI_Comm_dup(c_comm, &arg);
  sizepe = sizeof(MPI_Comm);
  i = data0->size[0];
  data0->size[0] = sizepe;
  emxEnsureCapacity_uint8_T(data0, i);
  i = dup->data->size[0];
  dup->data->size[0] = sizepe;
  emxEnsureCapacity_uint8_T(dup->data, i);
  for (i = 0; i < sizepe; i++) {
    dup->data->data[i] = data0->data[i];
  }

  emxFree_uint8_T(&data0);
  i = dup->type->size[0] * dup->type->size[1];
  dup->type->size[0] = 1;
  dup->type->size[1] = 8;
  emxEnsureCapacity_char_T(dup->type, i);
  for (i = 0; i < 8; i++) {
    dup->type->data[i] = x2[i];
  }
