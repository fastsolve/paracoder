                  ...
                  *toplevel)
{
  ...
  *info = MPI_Comm_dup(c_comm, &dup0);
  ...
  resultlen = dup->data->size[0];
  dup->data->size[0] = data0->size[0];
  emxEnsureCapacity((emxArray__common *)dup->data, resultlen, sizeof(unsigned
    char));
  loop_ub = data0->size[0];
  for (resultlen = 0; resultlen < loop_ub; resultlen++) {
    dup->data->data[resultlen] = data0->data[resultlen];
  }

  emxFree_uint8_T(&data0);
  resultlen = dup->type->size[0] * dup->type->size[1];
  dup->type->size[0] = 1;
  dup->type->size[1] = 8;
  emxEnsureCapacity((emxArray__common *)dup->type, resultlen, sizeof(char));
  for (resultlen = 0; resultlen < 8; resultlen++) {
    dup->type->data[resultlen] = t0_type[resultlen];
  }
