/*
 * mpi/codegen/lib/mpi_Isend/mpi_Isend_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Isend
 *
...
static void __mpi_Isend_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Isend:WrongInputType",
            "Input argument ptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Isend:WrongSizeOfInputArg",
    ...
    mpi_Isend(&ptr, count, &datatype, dest, tag, &comm, &req, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 6) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Isend:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Isend.\n");
        /* Call the API function. */
        __mpi_Isend_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Isend:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Isend.");
