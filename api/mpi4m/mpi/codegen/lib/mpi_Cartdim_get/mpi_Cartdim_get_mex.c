/*
 * mpi/codegen/lib/mpi_Cartdim_get/mpi_Cartdim_get_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Cartdim_get
 *
...
static void __mpi_Cartdim_get_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       comm;
    int32_T             *ndims;
    int32_T             *info;
    boolean_T           *toplevel;
    ...
    mpi_Cartdim_get(&comm, ndims, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&comm);
    plhs[0] = move_scalar_to_mxArray(ndims, mxINT32_CLASS);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Cartdim_get:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Cartdim_get.\n");
        /* Call the API function. */
        __mpi_Cartdim_get_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Cartdim_get:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Cartdim_get.");
