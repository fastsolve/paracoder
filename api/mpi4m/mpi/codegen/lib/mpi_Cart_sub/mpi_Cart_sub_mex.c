/*
 * mpi/codegen/lib/mpi_Cart_sub/mpi_Cart_sub_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Cart_sub
 *
 * C source code generated by m2c.
 * %#m2c options:7354ee3400045f48c0f1296a73b10fd8
 *
 */
...
static void __mpi_Cart_sub_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       comm;
    emxArray_int32_T     remain_dims;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Cart_sub:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Cart_sub:WrongSizeOfInputArg",
    ...
    if (mxGetNumberOfElements(prhs[1]) && mxGetClassID(prhs[1]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Cart_sub:WrongInputType",
            "Input argument remain_dims has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[1]) && mxGetDimensions(prhs[1])[1] != 1) 
        mexErrMsgIdAndTxt("mpi_Cart_sub:WrongSizeOfInputArg",
            "Dimension 2 of remain_dims should be equal to 1.");
    alias_mxArray_to_emxArray(prhs[1], (emxArray__common *)(&remain_dims), "remain_dims", 1);
    ...
    mpi_Cart_sub(&comm, &remain_dims, &newcomm, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&comm);
    free_emxArray((emxArray__common*)(&remain_dims));
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Cart_sub:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Cart_sub.\n");
        /* Call the API function. */
        __mpi_Cart_sub_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Cart_sub:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Cart_sub.");
