/*
 * mpi/codegen/lib/mpi_Startall/mpi_Startall_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Startall
 *
...
static void __mpi_Startall_api(mxArray **plhs, const mxArray ** prhs) {
    int32_T              count;
    M2C_OpaqueType       reqs;
    int32_T             *info;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Startall:WrongInputType",
            "Input argument count has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Startall:WrongSizeOfInputArg",
    ...
    marshallin_const_M2C_OpaqueType(&reqs, prhs[1], "reqs");
    ...
    mpi_Startall(count, &reqs, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    /* Nothing to be done for count */
    destroy_M2C_OpaqueType(&reqs);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("mpi_Startall:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Startall.\n");
        /* Call the API function. */
        __mpi_Startall_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Startall:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Startall.");
