/*
 * mpi/codegen/lib/mpi_Alltoall/mpi_Alltoall_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Alltoall
 *
...
static void __mpi_Alltoall_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Alltoall:WrongInputType",
            "Input argument sptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Alltoall:WrongSizeOfInputArg",
    ...
    mpi_Alltoall(&sptr, scount, &stype, &rptr, rcount, &rtype, &comm, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 7) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("mpi_Alltoall:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Alltoall.\n");
        /* Call the API function. */
        __mpi_Alltoall_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Alltoall:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Alltoall.");
