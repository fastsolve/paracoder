...
void mpi_Alltoall(const M2C_OpaquePtrType *sptr, int scount, const
                  ...
                  *comm, int *info, boolean_T *toplevel)
{
  ...
  if ((sptr->nbytes - sptr->offset < scount * nprocs * mpi_Type_size(stype->data,
        stype->type)) || (rptr->nbytes - rptr->offset < rcount * nprocs *
                          mpi_Type_size(rtype->data, rtype->type))) {
    c_m2c_error();
  }

  b_p = (sptr->data == rptr->data);
  if (b_p) {
    d_m2c_error();
  }
