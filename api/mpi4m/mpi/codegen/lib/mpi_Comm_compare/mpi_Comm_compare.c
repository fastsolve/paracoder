                      ...
                      int *result, int *info, boolean_T *toplevel)
{
  ...
  char * ptr;
  short unnamed_idx_1;
  p = (comm1->type->size[1] == 8);
  if (p && (comm1->type->size[1] != 0)) {
    resultlen = 0;
    exitg1 = false;
    while ((!exitg1) && (resultlen < 8)) {
      if (!(comm1->type->data[resultlen] == cv[resultlen])) {
        p = false;
        exitg1 = true;
      } else {
        resultlen++;
  ...
  comm = m2c_castdata(comm1->data);
  p = (comm2->type->size[1] == 8);
  if (p && (comm2->type->size[1] != 0)) {
    resultlen = 0;
    exitg1 = false;
    while ((!exitg1) && (resultlen < 8)) {
      if (!(comm2->type->data[resultlen] == cv[resultlen])) {
        p = false;
        exitg1 = true;
      } else {
        resultlen++;
  ...
  b_comm = m2c_castdata(comm2->data);
