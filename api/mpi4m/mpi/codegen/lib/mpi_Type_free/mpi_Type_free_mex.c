/*
 * mpi/codegen/lib/mpi_Type_free/mpi_Type_free_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Type_free
 *
...
static void __mpi_Type_free_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Type_free:WrongInputType",
            "Input argument datatype has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Type_free:WrongSizeOfInputArg",
    ...
    mpi_Type_free(&datatype, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Type_free:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Type_free.\n");
        /* Call the API function. */
        __mpi_Type_free_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Type_free:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Type_free.");
