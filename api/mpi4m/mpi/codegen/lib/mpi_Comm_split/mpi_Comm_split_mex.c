/*
 * mpi/codegen/lib/mpi_Comm_split/mpi_Comm_split_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Comm_split
 *
...
static void __mpi_Comm_split_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       comm;
    int32_T              color;
    int32_T              key;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Comm_split:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Comm_split:WrongSizeOfInputArg",
    ...
    if (mxGetNumberOfElements(prhs[1]) && mxGetClassID(prhs[1]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Comm_split:WrongInputType",
            "Input argument color has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[1]) != 1)
        mexErrMsgIdAndTxt("mpi_Comm_split:WrongSizeOfInputArg",
            "Argument color should be a scalar.");
    color = *(int32_T*)mxGetData(prhs[1]);

    if (mxGetNumberOfElements(prhs[2]) && mxGetClassID(prhs[2]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Comm_split:WrongInputType",
            "Input argument key has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[2]) != 1)
        mexErrMsgIdAndTxt("mpi_Comm_split:WrongSizeOfInputArg",
            "Argument key should be a scalar.");
    key = *(int32_T*)mxGetData(prhs[2]);
    ...
    mpi_Comm_split(&comm, color, key, &newcomm, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&comm);
    /* Nothing to be done for color */
    /* Nothing to be done for key */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Comm_split:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Comm_split.\n");
        /* Call the API function. */
        __mpi_Comm_split_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Comm_split:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Comm_split.");
