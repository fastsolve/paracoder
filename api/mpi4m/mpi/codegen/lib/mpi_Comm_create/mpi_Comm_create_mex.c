/*
 * mpi/codegen/lib/mpi_Comm_create/mpi_Comm_create_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Comm_create
 *
 * C source code generated by m2c.
 * %#m2c options:a09f62ecffa4bfb9af67e3b2538c8f26
 *
 */
...
static void __mpi_Comm_create_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       comm;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Comm_create:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Comm_create:WrongSizeOfInputArg",
    ...
    marshallin_const_M2C_OpaqueType(&group, prhs[1], "group");
    ...
    mpi_Comm_create(&comm, &group, &newcomm, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Comm_create:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Comm_create.\n");
        /* Call the API function. */
        __mpi_Comm_create_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Comm_create:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Comm_create.");
