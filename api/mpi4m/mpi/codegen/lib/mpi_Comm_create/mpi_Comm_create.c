                     ...
                     *newcomm, int *info, boolean_T *toplevel)
{
  ...
  if (b_p && (!(group->type->size[1] == 0))) {
    ...
    while ((!exitg1) && (resultlen < 9)) {
      if (!(group->type->data[resultlen] == cv1[resultlen])) {
        b_p = false;
        exitg1 = true;
      } else {
        resultlen++;
  ...
  if (!p) {
    ...
    for (resultlen = 0; resultlen < loop_ub; resultlen++) {
      b_comm->data[b_comm->size[0] * resultlen] = group->type->data[group->
        type->size[0] * resultlen];
    }

    b_comm->data[b_comm->size[0] * group->type->size[1]] = '\x00';
    b_m2c_error(b_comm);
  }
  ...
  *info = MPI_Comm_create(c_comm, obj, &newcomm0);
