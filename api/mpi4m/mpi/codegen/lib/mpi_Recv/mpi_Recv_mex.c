/*
 * mpi/codegen/lib/mpi_Recv/mpi_Recv_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Recv
 *
...
static void __mpi_Recv_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Recv:WrongInputType",
            "Input argument ptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Recv:WrongSizeOfInputArg",
    ...
    mpi_Recv(&ptr, count, &datatype, src, tag, &comm, &stat, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 6) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Recv:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Recv.\n");
        /* Call the API function. */
        __mpi_Recv_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Recv:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Recv.");
