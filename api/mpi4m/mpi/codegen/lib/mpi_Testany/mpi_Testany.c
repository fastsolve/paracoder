...
void mpi_Testany(int count, M2C_OpaqueType *reqs, int *idx, int *flag,
                 M2C_OpaqueType *stat, int *info, boolean_T *toplevel)
{
  ...
  *info = MPI_Testany(count, ptr, idx, flag, &arg);
...
void mpi_Testany_terminate(void)
{
}
