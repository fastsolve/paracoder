/*
 * mpi/codegen/lib/mpi_Testany/mpi_Testany_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Testany
 *
...
static void __mpi_Testany_api(mxArray **plhs, const mxArray ** prhs) {
    int32_T              count;
    M2C_OpaqueType       reqs;
    int32_T             *idx;
    int32_T             *flag;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Testany:WrongInputType",
            "Input argument count has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Testany:WrongSizeOfInputArg",
    ...
    mpi_Testany(count, &reqs, idx, flag, &stat, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    /* Nothing to be done for count */
    plhs[3] = marshallout_M2C_OpaqueType(&reqs);
    plhs[0] = move_scalar_to_mxArray(idx, mxINT32_CLASS);
    plhs[1] = move_scalar_to_mxArray(flag, mxINT32_CLASS);
    plhs[2] = marshallout_M2C_OpaqueType(&stat);
    plhs[4] = move_scalar_to_mxArray(info, mxINT32_CLASS);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 6)
            mexErrMsgIdAndTxt("mpi_Testany:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Testany.\n");
        /* Call the API function. */
        __mpi_Testany_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Testany:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Testany.");
