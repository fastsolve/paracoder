/*
 * mpi/codegen/lib/mpi_Cart_shift/mpi_Cart_shift_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Cart_shift
 *
...
static void __mpi_Cart_shift_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       comm;
    int32_T              direction;
    int32_T              displ;
    int32_T             *source;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Cart_shift:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Cart_shift:WrongSizeOfInputArg",
    ...
    if (mxGetNumberOfElements(prhs[1]) && mxGetClassID(prhs[1]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Cart_shift:WrongInputType",
            "Input argument direction has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[1]) != 1)
        mexErrMsgIdAndTxt("mpi_Cart_shift:WrongSizeOfInputArg",
            "Argument direction should be a scalar.");
    direction = *(int32_T*)mxGetData(prhs[1]);

    if (mxGetNumberOfElements(prhs[2]) && mxGetClassID(prhs[2]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Cart_shift:WrongInputType",
            "Input argument displ has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[2]) != 1)
        mexErrMsgIdAndTxt("mpi_Cart_shift:WrongSizeOfInputArg",
            "Argument displ should be a scalar.");
    displ = *(int32_T*)mxGetData(prhs[2]);

    source = (int32_T*)mxMalloc(sizeof(int32_T));

    dest = (int32_T*)mxMalloc(sizeof(int32_T));
    ...
    mpi_Cart_shift(&comm, direction, displ, source, dest, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&comm);
    /* Nothing to be done for direction */
    /* Nothing to be done for displ */
    plhs[0] = move_scalar_to_mxArray(source, mxINT32_CLASS);
    plhs[1] = move_scalar_to_mxArray(dest, mxINT32_CLASS);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 4)
            mexErrMsgIdAndTxt("mpi_Cart_shift:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Cart_shift.\n");
        /* Call the API function. */
        __mpi_Cart_shift_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Cart_shift:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Cart_shift.");
