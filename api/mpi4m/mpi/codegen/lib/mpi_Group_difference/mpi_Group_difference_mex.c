/*
 * mpi/codegen/lib/mpi_Group_difference/mpi_Group_difference_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Group_difference
 *
...
static void __mpi_Group_difference_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Group_difference:WrongInputType",
            "Input argument group1 has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Group_difference:WrongSizeOfInputArg",
    ...
    mpi_Group_difference(&group1, &group2, &newgroup, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Group_difference:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Group_difference.\n");
        /* Call the API function. */
        __mpi_Group_difference_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Group_difference:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Group_difference.");
