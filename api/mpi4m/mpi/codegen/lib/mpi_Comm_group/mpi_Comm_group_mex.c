/*
 * mpi/codegen/lib/mpi_Comm_group/mpi_Comm_group_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Comm_group
 *
...
static void __mpi_Comm_group_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       comm;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Comm_group:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Comm_group:WrongSizeOfInputArg",
    ...
    mpi_Comm_group(&comm, &group, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Comm_group:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Comm_group.\n");
        /* Call the API function. */
        __mpi_Comm_group_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Comm_group:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Comm_group.");
