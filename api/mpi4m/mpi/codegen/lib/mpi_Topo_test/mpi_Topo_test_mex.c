/*
 * mpi/codegen/lib/mpi_Topo_test/mpi_Topo_test_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Topo_test
 *
...
static void __mpi_Topo_test_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            comm;
    int32_T             *topo_type;
    int32_T             *info;
    boolean_T           *toplevel;
    ...
    mpi_Topo_test(&comm, topo_type, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_struct0_T(&comm);
    plhs[0] = move_scalar_to_mxArray(topo_type, mxINT32_CLASS);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Topo_test:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Topo_test.\n");
        /* Call the API function. */
        __mpi_Topo_test_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Topo_test:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Topo_test.");
