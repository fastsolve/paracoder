/*
 * mpi/codegen/lib/mpi_Cart_get/mpi_Cart_get_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Cart_get
 *
...
static void __mpi_Cart_get_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            comm;
    int32_T              maxdims;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Cart_get:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Cart_get:WrongSizeOfInputArg",
    ...
    maxdims = *(int32_T*)mxGetData(prhs[1]);
    init_emxArray((emxArray__common*)(&dims), 1);
    init_emxArray((emxArray__common*)(&periods), 1);
    ...
    mpi_Cart_get(&comm, maxdims, &dims, &periods, &coords, info, toplevel);
    ...
    mxFree(dims.size);
    plhs[1] = move_emxArray_to_mxArray((emxArray__common*)(&periods), mxINT32_CLASS);
    mxFree(periods.size);
    plhs[2] = move_emxArray_to_mxArray((emxArray__common*)(&coords), mxINT32_CLASS);
    mxFree(coords.size);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 5)
            mexErrMsgIdAndTxt("mpi_Cart_get:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Cart_get.\n");
        /* Call the API function. */
        __mpi_Cart_get_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Cart_get:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Cart_get.");
