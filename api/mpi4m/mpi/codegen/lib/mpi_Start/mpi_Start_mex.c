/*
 * mpi/codegen/lib/mpi_Start/mpi_Start_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Start
 *
...
static void __mpi_Start_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       req;
    int32_T             *info;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Start:WrongInputType",
            "Input argument req has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Start:WrongSizeOfInputArg",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("mpi_Start:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Start.\n");
        /* Call the API function. */
        __mpi_Start_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Start:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Start.");
