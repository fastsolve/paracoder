...
static void b_m2c_error(const emxArray_char_T *varargin_3)
{
  ...
  M2C_error("MPI:RuntimeError",
            "MPI_Graph_neighbors failed with error message %s\n",
            &b_varargin_3->data[0]);
  ...
  emxArray_int32_T *neighbors, int *info, boolean_T *toplevel)
{
  ...
  resultlen = neighbors->size[0];
  neighbors->size[0] = maxneighbors;
  emxEnsureCapacity((emxArray__common *)neighbors, resultlen, sizeof(int));
  for (resultlen = 0; resultlen < maxneighbors; resultlen++) {
    neighbors->data[resultlen] = 0;
  }
  ...
  *info = MPI_Graph_neighbors(c_comm, rank, maxneighbors, &neighbors->data[0]);
...
void mpi_Graph_neighbors_initialize(void)
{
}

void mpi_Graph_neighbors_terminate(void)
{
}
