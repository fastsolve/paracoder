/*
 * mpi/codegen/lib/mpi_Testsome/mpi_Testsome_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Testsome
 *
...
static void __mpi_Testsome_api(mxArray **plhs, const mxArray ** prhs) {
    int32_T              count;
    struct0_T            reqs;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Testsome:WrongInputType",
            "Input argument count has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Testsome:WrongSizeOfInputArg",
    ...
    mpi_Testsome(count, &reqs, &idxs, &stats, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 5)
            mexErrMsgIdAndTxt("mpi_Testsome:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Testsome.\n");
        /* Call the API function. */
        __mpi_Testsome_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Testsome:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Testsome.");
