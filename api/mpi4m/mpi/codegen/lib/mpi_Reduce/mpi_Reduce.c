                ...
                struct1_T *comm, int *info, boolean_T *toplevel)
{
  ...
  if (!p) {
    ...
    for (resultlen = 0; resultlen < loop_ub; resultlen++) {
      b_comm->data[b_comm->size[0] * resultlen] = datatype->type->data
        [datatype->type->size[0] * resultlen];
    }

    b_comm->data[b_comm->size[0] * datatype->type->size[1]] = '\x00';
    m2c_error(b_comm);
  }
  ...
  if (!p) {
    ...
    for (resultlen = 0; resultlen < loop_ub; resultlen++) {
      b_comm->data[b_comm->size[0] * resultlen] = op->type->data[op->type->size
        [0] * resultlen];
    }

    b_comm->data[b_comm->size[0] * op->type->size[1]] = '\x00';
    e_m2c_error(b_comm);
  }
