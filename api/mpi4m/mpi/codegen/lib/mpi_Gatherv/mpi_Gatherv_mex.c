/*
 * mpi/codegen/lib/mpi_Gatherv/mpi_Gatherv_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Gatherv
 *
 * C source code generated by m2c.
 * %#m2c options:3180baf54eddde99f1a18441a1bab6d4
 *
 */
...
static void __mpi_Gatherv_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Gatherv:WrongInputType",
            "Input argument sptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Gatherv:WrongSizeOfInputArg",
    ...
    mpi_Gatherv(&sptr, scount, &stype, &rptr, &rcounts, &disps, &rtype, root, &comm, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 9) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("mpi_Gatherv:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Gatherv.\n");
        /* Call the API function. */
        __mpi_Gatherv_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Gatherv:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Gatherv.");
