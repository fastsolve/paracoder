/*
 * mpi/codegen/lib/mpi_Issend/mpi_Issend_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Issend
 *
...
static void __mpi_Issend_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Issend:WrongInputType",
            "Input argument ptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Issend:WrongSizeOfInputArg",
    ...
    mpi_Issend(&ptr, count, &datatype, dest, tag, &comm, &req, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 6) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Issend:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Issend.\n");
        /* Call the API function. */
        __mpi_Issend_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Issend:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Issend.");
