/*
 * mpi/codegen/lib/mpi_Probe/mpi_Probe_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Probe
 *
...
static void __mpi_Probe_api(mxArray **plhs, const mxArray ** prhs) {
    int32_T              source;
    int32_T              tag;
    M2C_OpaqueType       comm;
    M2C_OpaqueType       stat;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("mpi_Probe:WrongInputType",
            "Input argument source has incorrect data type; int32 is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Probe:WrongSizeOfInputArg",
    ...
    mpi_Probe(source, tag, &comm, &stat, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Probe:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Probe.\n");
        /* Call the API function. */
        __mpi_Probe_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Probe:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Probe.");
