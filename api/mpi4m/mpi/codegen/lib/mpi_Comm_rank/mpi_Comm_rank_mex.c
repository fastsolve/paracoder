...
static void __mpi_Comm_rank_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            comm;
    int32_T             *rank;
    int32_T             *info;
    boolean_T           *toplevel;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Comm_rank:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Comm_rank.\n");
        /* Call the API function. */
        __mpi_Comm_rank_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Comm_rank:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Comm_rank.");
