#include "mpi_Graph_neighbors_count.h"
...
static void b_m2c_error(const emxArray_char_T *varargin_3)
{
  ...
  M2C_error("MPI:RuntimeError",
            "MPI_Graph_neighbors_count failed with error message %s\n",
            &b_varargin_3->data[0]);
...
void mpi_Graph_neighbors_count(const M2C_OpaqueType *comm, int rank,
                               int *nneighbors, int *info, boolean_T *toplevel)
{
  ...
  *info = MPI_Graph_neighbors_count(c_comm, rank, nneighbors);
  *toplevel = true;
  if (*info != 0) {
...
void mpi_Graph_neighbors_count_initialize(void)
{
}

void mpi_Graph_neighbors_count_terminate(void)
{
}
