/*
 * mpi/codegen/lib/mpi_Graph_neighbors_count/mpi_Graph_neighbors_count_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Graph_neighbors_count
 *
...
static void __mpi_Graph_neighbors_count_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            comm;
    int32_T              rank;
    int32_T             *nneighbors;
    int32_T             *info;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Graph_neighbors_count:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Graph_neighbors_count:WrongSizeOfInputArg",
    ...
    nneighbors = (int32_T*)mxMalloc(sizeof(int32_T));
    ...
    mpi_Graph_neighbors_count(&comm, rank, nneighbors, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_struct0_T(&comm);
    /* Nothing to be done for rank */
    plhs[0] = move_scalar_to_mxArray(nneighbors, mxINT32_CLASS);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Graph_neighbors_count:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Graph_neighbors_count.\n");
        /* Call the API function. */
        __mpi_Graph_neighbors_count_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Graph_neighbors_count:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Graph_neighbors_count.");
