...
void mpi_Comm_remote_group(const struct0_T *comm, struct0_T *group, int *info,
  boolean_T *toplevel)
{
  MPI_Group group0;
  boolean_T p;
  ...
  *info = MPI_Comm_remote_group(c_comm, &group0);
  *toplevel = true;
  ...
  i0 = group->data->size[0];
  group->data->size[0] = data0->size[0];
  emxEnsureCapacity((emxArray__common *)group->data, i0, (int)sizeof(unsigned
    char));
  resultlen = data0->size[0];
  for (i0 = 0; i0 < resultlen; i0++) {
    group->data->data[i0] = data0->data[i0];
  }

  emxFree_uint8_T(&data0);
  i0 = group->type->size[0] * group->type->size[1];
  group->type->size[0] = 1;
  group->type->size[1] = 9;
  emxEnsureCapacity((emxArray__common *)group->type, i0, (int)sizeof(char));
  for (i0 = 0; i0 < 9; i0++) {
    group->type->data[i0] = t0_type[i0];
  }
