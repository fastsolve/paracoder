/*
 * mpi/codegen/lib/mpi_Wait/mpi_Wait_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Wait
 *
...
static void __mpi_Wait_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       req;
    M2C_OpaqueType       stat;
    int32_T             *info;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Wait:WrongInputType",
            "Input argument req has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Wait:WrongSizeOfInputArg",
    ...
    mpi_Wait(&req, &stat, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    plhs[1] = marshallout_M2C_OpaqueType(&req);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 4)
            mexErrMsgIdAndTxt("mpi_Wait:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Wait.\n");
        /* Call the API function. */
        __mpi_Wait_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Wait:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Wait.");
