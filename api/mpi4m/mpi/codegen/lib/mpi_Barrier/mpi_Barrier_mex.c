...
static void __mpi_Barrier_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            comm;
    int32_T             *info;
    boolean_T           *toplevel;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("mpi_Barrier:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Barrier.\n");
        /* Call the API function. */
        __mpi_Barrier_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Barrier:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Barrier.");
