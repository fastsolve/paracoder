/*
 * mpi/codegen/lib/mpi_Cancel/mpi_Cancel_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Cancel
 *
...
static void __mpi_Cancel_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       req;
    int32_T             *info;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Cancel:WrongInputType",
            "Input argument req has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Cancel:WrongSizeOfInputArg",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("mpi_Cancel:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Cancel.\n");
        /* Call the API function. */
        __mpi_Cancel_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Cancel:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Cancel.");
