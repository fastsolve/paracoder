/*
 * mpi/codegen/lib/mpi_Request_get_status/mpi_Request_get_status_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Request_get_status
 *
...
static void __mpi_Request_get_status_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       req;
    int32_T             *flag;
    M2C_OpaqueType       stat;
    int32_T             *info;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Request_get_status:WrongInputType",
            "Input argument req has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Request_get_status:WrongSizeOfInputArg",
            "Argument req should be a scalar.");
    marshallin_const_M2C_OpaqueType(&req, prhs[0], "req");
    ...
    mpi_Request_get_status(&req, flag, &stat, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&req);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 4)
            mexErrMsgIdAndTxt("mpi_Request_get_status:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Request_get_status.\n");
        /* Call the API function. */
        __mpi_Request_get_status_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Request_get_status:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Request_get_status.");
