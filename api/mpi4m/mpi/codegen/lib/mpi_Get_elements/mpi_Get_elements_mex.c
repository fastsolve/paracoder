/*
 * mpi/codegen/lib/mpi_Get_elements/mpi_Get_elements_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Get_elements
 *
...
static void __mpi_Get_elements_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       stat;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Get_elements:WrongInputType",
            "Input argument stat has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Get_elements:WrongSizeOfInputArg",
    ...
    marshallin_const_M2C_OpaqueType(&datatype, prhs[1], "datatype");

    elems = (int32_T*)mxMalloc(sizeof(int32_T));
    ...
    mpi_Get_elements(&stat, &datatype, elems, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    destroy_M2C_OpaqueType(&stat);
    destroy_M2C_OpaqueType(&datatype);
    plhs[0] = move_scalar_to_mxArray(elems, mxINT32_CLASS);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Get_elements:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Get_elements.\n");
        /* Call the API function. */
        __mpi_Get_elements_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Get_elements:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Get_elements.");
