/*
 * mpi/codegen/lib/mpi_Bsend_init/mpi_Bsend_init_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Bsend_init
 *
...
static void __mpi_Bsend_init_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Bsend_init:WrongInputType",
            "Input argument ptr has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Bsend_init:WrongSizeOfInputArg",
    ...
    mpi_Bsend_init(&ptr, count, &datatype, dest, tag, &comm, &req, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 6) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Bsend_init:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Bsend_init.\n");
        /* Call the API function. */
        __mpi_Bsend_init_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Bsend_init:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Bsend_init.");
