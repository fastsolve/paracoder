/*
 * mpi/codegen/lib/mpi_Graph_create/mpi_Graph_create_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Graph_create
 *
...
static void __mpi_Graph_create_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       comm;
    int32_T              nnodes;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Graph_create:WrongInputType",
    ...
    mpi_Graph_create(&comm, nnodes, &b_index, &edges, reorder, &newcomm, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 5) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Graph_create:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Graph_create.\n");
        /* Call the API function. */
        __mpi_Graph_create_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Graph_create:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Graph_create.");
