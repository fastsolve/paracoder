/*
 * mpi/codegen/lib/mpi_Test/mpi_Test_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Test
 *
...
static void __mpi_Test_api(mxArray **plhs, const mxArray ** prhs) {
    M2C_OpaqueType       req;
    int32_T             *flag;
    M2C_OpaqueType       stat;
    int32_T             *info;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Test:WrongInputType",
            "Input argument req has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Test:WrongSizeOfInputArg",
    ...
    mpi_Test(&req, flag, &stat, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    plhs[2] = marshallout_M2C_OpaqueType(&req);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 5)
            mexErrMsgIdAndTxt("mpi_Test:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Test.\n");
        /* Call the API function. */
        __mpi_Test_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Test:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Test.");
