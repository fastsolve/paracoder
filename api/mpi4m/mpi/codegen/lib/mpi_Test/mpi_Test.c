...
void mpi_Test(struct0_T *req, int *flag, struct0_T *stat, int *info, boolean_T
              *toplevel)
{
  boolean_T p;
  boolean_T b_p;
  ...
  *info = MPI_Test(&t_req, flag, &stat0);
  ...
  for (resultlen = 0; resultlen < 10; resultlen++) {
    t1_type[resultlen] = cv1[resultlen];
  }
  ...
  for (resultlen = 0; resultlen < 10; resultlen++) {
    stat->type->data[resultlen] = t1_type[resultlen];
  }
