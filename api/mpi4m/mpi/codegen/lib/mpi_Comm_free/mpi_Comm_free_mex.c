/*
 * mpi/codegen/lib/mpi_Comm_free/mpi_Comm_free_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Comm_free
 *
...
static void __mpi_Comm_free_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            comm;
    int32_T             *info;
    boolean_T           *toplevel;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Comm_free:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Comm_free:WrongSizeOfInputArg",
            "Argument comm should be a scalar.");
    marshallin_struct0_T(&comm, prhs[0], "comm");
    ...
    mpi_Comm_free(&comm, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Comm_free:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Comm_free.\n");
        /* Call the API function. */
        __mpi_Comm_free_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Comm_free:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Comm_free.");
