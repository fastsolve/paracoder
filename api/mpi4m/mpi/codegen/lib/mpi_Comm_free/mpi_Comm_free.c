...
void mpi_Comm_free(struct0_T *comm, int *info, boolean_T *toplevel)
{
  boolean_T p;
  boolean_T b_p;
  int resultlen;
  ...
  *toplevel = true;
  ...
  emxEnsureCapacity((emxArray__common *)comm->data, i2, (int)sizeof(unsigned
    char));
  resultlen = data0->size[0];
  for (i2 = 0; i2 < resultlen; i2++) {
    comm->data->data[i2] = data0->data[i2];
  }

  emxFree_uint8_T(&data0);
  i2 = comm->type->size[0] * comm->type->size[1];
  comm->type->size[0] = 1;
  comm->type->size[1] = 8;
  emxEnsureCapacity((emxArray__common *)comm->type, i2, (int)sizeof(char));
  for (i2 = 0; i2 < 8; i2++) {
    comm->type->data[i2] = t0_type[i2];
  }
...
void mpi_Comm_free_initialize(void)
{
}

void mpi_Comm_free_terminate(void)
{
}
