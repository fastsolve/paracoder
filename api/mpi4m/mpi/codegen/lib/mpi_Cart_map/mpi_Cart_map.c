...
void mpi_Cart_map(const M2C_OpaqueType *comm, int ndims,
                  const emxArray_int32_T *dims, const emxArray_int32_T *periods,
                  int *newrank, int *info, boolean_T *toplevel)
{
  ...
  *info =
      MPI_Cart_map(c_comm, ndims, &dims->data[0], &periods->data[0], newrank);
  *toplevel = true;
  if (*info != 0) {
