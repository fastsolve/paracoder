/*
 * mpi/codegen/lib/mpi_Cart_map/mpi_Cart_map_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Cart_map
 *
...
static void __mpi_Cart_map_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            comm;
    int32_T              ndims;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Cart_map:WrongInputType",
            "Input argument comm has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Cart_map:WrongSizeOfInputArg",
    ...
    newrank = (int32_T*)mxMalloc(sizeof(int32_T));
    ...
    mpi_Cart_map(&comm, ndims, &dims, &periods, newrank, info, toplevel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 4) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Cart_map:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Cart_map.\n");
        /* Call the API function. */
        __mpi_Cart_map_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Cart_map:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Cart_map.");
