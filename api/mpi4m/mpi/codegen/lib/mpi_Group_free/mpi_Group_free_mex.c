/*
 * mpi/codegen/lib/mpi_Group_free/mpi_Group_free_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Group_free
 *
...
static void __mpi_Group_free_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Group_free:WrongInputType",
            "Input argument group has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Group_free:WrongSizeOfInputArg",
            "Argument group should be a scalar.");
    marshallin_M2C_OpaqueType(&group, prhs[0], "group");
    ...
    mpi_Group_free(&group, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    plhs[0] = marshallout_M2C_OpaqueType(&group);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Group_free:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Group_free.\n");
        /* Call the API function. */
        __mpi_Group_free_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Group_free:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Group_free.");
