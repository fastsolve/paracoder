  ...
  *extent, int *info, boolean_T *toplevel)
{
  ...
  *info = MPI_Type_get_extent(c_datatype, &lb0, &extent0);
  ...
  emxEnsureCapacity((emxArray__common *)extent->data, resultlen, sizeof(unsigned
    char));
  ...
  emxEnsureCapacity((emxArray__common *)extent->type, resultlen, sizeof(char));
  for (resultlen = 0; resultlen < 8; resultlen++) {
  ...
  emxEnsureCapacity((emxArray__common *)lb->data, resultlen, sizeof(unsigned
    char));
  loop_ub = data0->size[0];
  for (resultlen = 0; resultlen < loop_ub; resultlen++) {
  ...
  lb->type->size[0] = 1;
  lb->type->size[1] = 8;
  emxEnsureCapacity((emxArray__common *)lb->type, resultlen, sizeof(char));
  for (resultlen = 0; resultlen < 8; resultlen++) {
