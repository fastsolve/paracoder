/*
 * mpi/codegen/lib/mpi_Type_commit/mpi_Type_commit_mex.c
 *
 * Auxiliary code for mexFunction of mpi_Type_commit
 *
...
static void __mpi_Type_commit_api(mxArray **plhs, const mxArray ** prhs) {
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("mpi_Type_commit:WrongInputType",
            "Input argument datatype has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("mpi_Type_commit:WrongSizeOfInputArg",
            "Argument datatype should be a scalar.");
    marshallin_M2C_OpaqueType(&datatype, prhs[0], "datatype");
    ...
    mpi_Type_commit(&datatype, info, toplevel);

    /* Deallocate input and marshall out function outputs */
    plhs[0] = marshallout_M2C_OpaqueType(&datatype);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("mpi_Type_commit:TooManyOutputArguments",
                "Too many output arguments for entry-point mpi_Type_commit.\n");
        /* Call the API function. */
        __mpi_Type_commit_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("mpi_Type_commit:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point mpi_Type_commit.");
