...
void mpi_Type_commit(struct0_T *datatype, int *info, boolean_T *toplevel)
{
  boolean_T p;
  boolean_T b_p;
  int resultlen;
  ...
  *info = MPI_Type_commit(&t_datatype);
  *toplevel = true;
  ...
  emxEnsureCapacity((emxArray__common *)datatype->data, resultlen, sizeof
                    (unsigned char));
  ...
  emxEnsureCapacity((emxArray__common *)datatype->type, resultlen, sizeof(char));
  for (resultlen = 0; resultlen < 12; resultlen++) {
