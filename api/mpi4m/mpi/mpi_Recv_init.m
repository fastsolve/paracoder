function [req, info, toplevel] = mpi_Recv_init(ptr, count, datatype, dest, tag, comm)
%mpi_Recv_init	Create a persistent request for a receive
%
%  [req, info] = mpi_Recv_init(ptr, count, datatype, dest, tag, comm)
%
...
%      mpi_Waitsome, mpi_Recv
%
% C interface:
%   int MPI_Recv_init(void *buf, int count, MPI_Datatype datatype, int source, 
%                     int tag, MPI_Comm comm, MPI_Request *request)
% http://mpi.deino.net/mpi_functions/MPI_Recv_init.html
...
if isstruct(ptr) && coder.ismatlabthread
    if (ptr.nbytes-ptr.offset < count*mpi_Type_size(datatype))
        m2c_error('mpi_Recv_init:OutOfBound','Message size is larger than variable size.');
    end
end

req0 = coder.opaque('MPI_Request');
info = coder.ceval('MPI_Recv_init', m2c_opaque_ptr(ptr), int32(count), ...
...
if info && (toplevel || m2c_debug)
    m2c_error('MPI:RuntimeError', 'MPI_Recv_init failed with error message %s\n', mpi_Error_string(info))
end
...
%! var2 = zeros(10,10);
%!
%! [req1,info] = mpi_Isend(m2c_opaque_ptr_const(var1), count, MPI_DOUBLE, int32(0), int32(2), comm);
%! assert(info==0);
%! [req2, info] = mpi_Recv_init(m2c_opaque_ptr(var2), count, MPI_DOUBLE, int32(0), int32(2), comm);
%! assert(info==0);
%! info = mpi_Start(req2);
%! assert(info==0);
