  ...
  const struct0_T *oldtype, struct0_T *newtype, int *info, boolean_T *toplevel)
{
  ...
  if (!p) {
    ...
    i0 = b_stride->size[0] * b_stride->size[1];
    b_stride->size[0] = 1;
    b_stride->size[1] = stride->type->size[1] + 1;
    emxEnsureCapacity((emxArray__common *)b_stride, i0, (int)sizeof(char));
    resultlen = stride->type->size[1];
    for (i0 = 0; i0 < resultlen; i0++) {
      b_stride->data[b_stride->size[0] * i0] = stride->type->data[stride->
        type->size[0] * i0];
    }
  ...
  resultlen = stride->data->size[0];
  for (i0 = 0; i0 < resultlen; i0++) {
    data0->data[i0] = stride->data->data[i0];
  }
