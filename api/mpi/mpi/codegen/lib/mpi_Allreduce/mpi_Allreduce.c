                   ...
                   struct1_T *comm, int *info, boolean_T *toplevel)
{
  ...
  if (emlrtIsMATLABThread(emlrtRootTLSGlobal) && (((sptr->nbytes != 0) &&
        (sptr->nbytes - sptr->offset < count * mpi_Type_size(datatype->data,
          datatype->type))) || ((rptr->nbytes != 0) && (rptr->nbytes -
         rptr->offset < count * mpi_Type_size(datatype->data, datatype->type)))))
  {
    b_m2c_error();
  }
  ...
  for (i0 = 0; i0 < resultlen; i0++) {
    b_data->data[i0] = datatype->data->data[i0];
  }
  ...
  for (i0 = 0; i0 < resultlen; i0++) {
    b_data->data[i0] = op->data->data[i0];
  }
  ...
  *info = MPI_Allreduce(ptr, b_ptr, count, c_datatype, obj, c_comm);
  *toplevel = true;
