...
void mpi_Waitsome(int count, struct0_T *reqs, emxArray_int32_T *idxs, struct0_T *
                  stats, int *info, boolean_T *toplevel)
{
  ...
  i2 = idxs->size[0];
  idxs->size[0] = count;
  emxEnsureCapacity((emxArray__common *)idxs, i2, (int)sizeof(int));
  ...
  *info = MPI_Waitsome(count, ptr, &n, &idxs->data[0], b_ptr);
  *toplevel = true;
...
void mpi_Waitsome_initialize(void)
{
}

void mpi_Waitsome_terminate(void)
{
}
