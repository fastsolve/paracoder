...
static void b_m2c_error(const emxArray_char_T *varargin_3)
{
  ...
  M2C_error("MPI:RuntimeError",
            "MPI_Comm_call_errhandler failed with error message %s\n",
            &b_varargin_3->data[0]);
...
void mpi_Comm_call_errhandler(const struct0_T *comm, int errorcode, int *info,
  boolean_T *toplevel)
{
  boolean_T p;
  boolean_T b_p;
  ...
  *info = MPI_Comm_call_errhandler(c_comm, errorcode);
  *toplevel = true;
...
void mpi_Comm_call_errhandler_initialize(void)
{
}

void mpi_Comm_call_errhandler_terminate(void)
{
}
