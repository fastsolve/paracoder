...
void mpi_Scan(const struct0_T *sptr, const struct0_T *rptr, int count, const
              struct1_T *datatype, const struct1_T *op, const struct1_T *comm,
              int *info, boolean_T *toplevel)
{
  ...
  if (emlrtIsMATLABThread(emlrtRootTLSGlobal)) {
    ...
    if (sptr->nbytes - sptr->offset < count * size) {
      b_m2c_error();
    }
  }
  ...
  if (!p) {
    ...
    i0 = d_datatype->size[0] * d_datatype->size[1];
    d_datatype->size[0] = 1;
    d_datatype->size[1] = datatype->type->size[1] + 1;
    emxEnsureCapacity((emxArray__common *)d_datatype, i0, (int)sizeof(char));
    resultlen = datatype->type->size[1];
    for (i0 = 0; i0 < resultlen; i0++) {
      d_datatype->data[d_datatype->size[0] * i0] = datatype->type->data
  ...
  *info = MPI_Scan(ptr, b_ptr, count, c_datatype, obj, c_comm);
  *toplevel = true;
...
void mpi_Scan_initialize(void)
{
}

void mpi_Scan_terminate(void)
{
}
