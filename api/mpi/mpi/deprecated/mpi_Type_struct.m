function [newtype, info, toplevel] = mpi_Type_struct(count, blocklens, indices, oldtypes)
%mpi_Type_struct   Creates a struct datatype
%
%  [newtype, info] = mpi_Type_struct(count, blocklens, indices, oldtypes)
%
%Input arguments:
...
%
% The MPI-2 standard deprecated this routine. Its replacement is 
%     MPI_Type_create_struct
%
% SEE ALSO   mpi_Type_indexed, MPI_Type_hindexed, mpi_Type_free
%
% C interface:
% int MPI_Type_struct(int count, int blocklens[], MPI_Aint indices[],
%     MPI_Datatype oldtype[], MPI_Datatype *newtype);
% http://mpi.deino.net/mpi_functions/MPI_Type_struct.html
...
indices0 = m2c_unalias(indices0); oldtypes0 = m2c_unalias(oldtypes0);  
info = coder.ceval('MPI_Type_struct', int32(count), coder.rref(blocklens), ...
    m2c_opaque_ptr_const(indices0, 'MPI_Aint *'), ...
    m2c_opaque_ptr_const(oldtypes0, 'MPI_Datatype *'), coder.wref(newtype0));
indices0 = m2c_unalias(indices0); oldtypes0 = m2c_unalias(oldtypes0);  %#ok<NASGU>

toplevel = nargout>2;
newtype = MPI_Datatype(newtype0, toplevel);

if info && (toplevel || m2c_debug)
    m2c_error('MPI:RuntimeError', 'MPI_Type_struct returned error code %d\n', info)
end
...
%! newtype = mpi_Type_struct(int32(2), blocklens, indices, oldtype);
