function [newtype, info, toplevel] = mpi_Type_create_hvector(count, blocklen, stride, oldtype)
%mpi_Type_create_hvector   Creates a vector (strided) datatype with offset in bytes
%
%  [newtype, info] = mpi_Type_create_hvector(count, blocklen, stride, oldtype)
%
%Input arguments:
...
%
% NOTE: stride must be representable by int32.
%
% C interface:
% int MPI_Type_create_hvector(int count,int blocklen, MPI_Aint stride, 
%                      MPI_Datatype oldtype, MPI_Datatype *newtype)
% http://mpi.deino.net/mpi_functions/MPI_Type_create_hvector.html
...
info = coder.ceval('MPI_Type_create_hvector', count, blocklen, ...
...
if info && (toplevel || m2c_debug)
    m2c_error('MPI:RuntimeError', 'MPI_Type_create_hvector failed with error message %s\n', mpi_Error_string(info))
end
...
%! newtype = mpi_Type_create_hvector(int32(1), int32(10), MPI_Aint(80), MPI_DOUBLE);
