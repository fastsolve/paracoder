function [req, info, toplevel] = mpi_Isend(ptr, count, datatype, dest, tag, comm)
%mpi_Isend    Begins a nonblocking send
%
%  [req, info] = mpi_Isend(ptr, count, datatype, dest, tag, comm)
%
%Input arguments:
...
%
% SEE ALSO: mpi_Send, mpi_Test, mpi_Testall, mpi_Testany,
...
if isstruct(ptr) && coder.ismatlabthread
    if (ptr.nbytes-ptr.offset < count*mpi_Type_size(datatype))
        m2c_error('mpi_Isend:OutOfBound','Message size is larger than variable size.');
    end
end

req0 = coder.opaque('MPI_Request');
info = coder.ceval('MPI_Isend', m2c_opaque_ptr_const(ptr), ...
    int32(count), MPI_Datatype(datatype), int32(dest), int32(tag), ...
    MPI_Comm(comm), coder.wref(req0));

toplevel = nargout>2;
req = MPI_Request(req0, toplevel);

if info && (toplevel || m2c_debug)
    m2c_error('MPI:RuntimeError', 'MPI_Isend failed with error message %s\n', mpi_Error_string(info))
end
...
%! var2 = zeros(10,10);
%!
%! [req1,info] = mpi_Isend(m2c_opaque_ptr_const(var1), count, MPI_DOUBLE, int32(0), int32(1), comm);
%! assert(info==0);
%! [~,info] = mpi_Recv(m2c_opaque_ptr(var2), count, MPI_DOUBLE, int32(0), int32(1), comm);
%! mpi_Wait(req1);
%! assert(info==0);
