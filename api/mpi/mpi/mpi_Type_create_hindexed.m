function [newtype, info, toplevel] = mpi_Type_create_hindexed(count, blocklens, indices, oldtype)
%mpi_Type_create_hindexed   Creates an indexed datatype with offsets in bytes
%
%  [newtype, info] = mpi_Type_create_hindexed(count, blocklens, indices, oldtype)
%
%Input arguments:
...
%
% SEE ALSO   mpi_Type_indexed, MPI_Type_struct, mpi_Type_free
%
% C interface:
% int MPI_Type_create_hindexed(int count, int blocklens[], MPI_Aint indices[], ...
...
info = coder.ceval('MPI_Type_create_hindexed', int32(count), coder.rref(blocklens), ...
...
if info && (toplevel || m2c_debug)
    m2c_error('MPI:RuntimeError', 'MPI_Type_create_hindexed failed with error message %s\n', mpi_Error_string(info))
end
...
%! newtype = mpi_Type_create_hindexed(int32(2), blocklens, indices, MPI_DOUBLE);
