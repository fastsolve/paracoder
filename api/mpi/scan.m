function [bglobal, info] = scan(blocal, sz_in, op, comm, root) %#codegen
%SCAN Perform a scan operation to a root process.
%   [bglobal, info] = scan(blocal, sz, op, comm [, root])
% Scan to the root process for sz elements in blocal. If sz is empty, 
% it will be set to numel(blocal). The default root is 0.
%
%See also reduce, allreduce, bcast
...
info = mpi_Scan(m2c_opaque_ptr_const(blocal), m2c_opaque_ptr(bglobal), ...
...
%! [rvar, info] = scan(svar, [], MPI_SUM, comm);
%! assert(info==0);
