                               ...
                               int cend)
{
  ...
  for (jj = cstart; jj <= cend; jj++) {
    ...
    for (ii = cstart; ii <= i; ii++) {
      bs->data[jj - 1] -=
          R->data[(ii + R->size[0] * (jj - 1)) - 1] * bs->data[ii - 1];
    }
    bs->data[jj - 1] /= R->data[(jj + R->size[0] * (jj - 1)) - 1];
  }
}

double condest_triu(const emxArray_real_T *R, int m, int maxiter,
                    CondestBuf *buf)
{
  double a;
  double kappa;
...
void condest_triu_3args(const emxArray_real_T *R, int m, int maxiter,
