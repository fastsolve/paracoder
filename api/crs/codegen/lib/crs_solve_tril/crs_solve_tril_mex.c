/*
 * crs/codegen/lib/crs_solve_tril/crs_solve_tril_mex.c
 *
 * Auxiliary code for mexFunction of crs_solve_tril
 *
...
static void __crs_solve_tril_api(mxArray **plhs, const mxArray ** prhs) {
    CRS_Matrix           A;
    emxArray_real_T      b;

    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("crs_solve_tril:WrongInputType",
            "Input argument A has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("crs_solve_tril:WrongSizeOfInputArg",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 2) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("crs_solve_tril:TooManyOutputArguments",
                "Too many output arguments for entry-point crs_solve_tril.\n");
        /* Call the API function. */
        __crs_solve_tril_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("crs_solve_tril:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point crs_solve_tril.");
}
