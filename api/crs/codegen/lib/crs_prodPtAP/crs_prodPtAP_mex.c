...
static void __crs_prodPtAP_api(mxArray **plhs, const mxArray ** prhs) {
    CRS_Matrix           A;
    CRS_Matrix           P;
    CRS_Matrix           B;
    ...
    if (mxGetNumberOfElements(prhs[1]) != 1)
        mexErrMsgIdAndTxt("crs_prodPtAP:WrongSizeOfInputArg",
            "Argument P should be a scalar.");
    marshallin_const_CRS_Matrix(&P, prhs[1], "P");
    prealloc_CRS_Matrix(&B);
    ...
    destroy_CRS_Matrix(&P);
    plhs[0] = marshallout_CRS_Matrix(&B);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 2) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("crs_prodPtAP:TooManyOutputArguments",
                "Too many output arguments for entry-point crs_prodPtAP.\n");
        /* Call the API function. */
        __crs_prodPtAP_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("crs_prodPtAP:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point crs_prodPtAP.");
}
