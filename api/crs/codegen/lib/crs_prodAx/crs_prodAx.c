...
void crs_prodAx(const CRS_Matrix *A, const emxArray_real_T *x, emxArray_real_T
                *b, int nthreads)
{
  int n;
...
#pragma omp parallel default(shared) num_threads(nthreads)
  {
    n = omp_get_num_threads();
    crs_prodAx_kernel(A->row_ptr, A->col_ind, A->val, x, x->size[0], b, b->size[0],
                    ...
                    emxArray_real_T *b)
{
  ...
  b_crs_prodAx_kernel(A->row_ptr, A->col_ind, A->val, x, x->size[0], b, A->nrows,
                      A->nrows, x->size[1]);
}
                     ...
                     emxArray_real_T *b)
{
  ...
  b_crs_prodAx_kernel(A->row_ptr, A->col_ind, A->val, x, x->size[0], b, b->size
                      [0], A->nrows, x->size[1]);
}
