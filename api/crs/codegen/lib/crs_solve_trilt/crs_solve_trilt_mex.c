/*
 * crs/codegen/lib/crs_solve_trilt/crs_solve_trilt_mex.c
 *
 * Auxiliary code for mexFunction of crs_solve_trilt
 *
...
static void __crs_solve_trilt_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            A;
    emxArray_real_T      b;

    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("crs_solve_trilt:WrongInputType",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 2) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("crs_solve_trilt:TooManyOutputArguments",
                "Too many output arguments for entry-point crs_solve_trilt.\n");
        /* Call the API function. */
        __crs_solve_trilt_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("crs_solve_trilt:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point crs_solve_trilt.");
}
