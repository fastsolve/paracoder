/*
 * crs/codegen/lib/crs_solve_triut/crs_solve_triut_mex.c
 *
 * Auxiliary code for mexFunction of crs_solve_triut
 *
 * C source code generated by m2c.
 * %#m2c options:e68ac978992782210bc2e3e5ed159946
 *
 */
...
static void __crs_solve_triut_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            A;
    emxArray_real_T      b;

    /* Marshall in inputs and preallocate outputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("crs_solve_triut:WrongInputType",
            "Input argument A has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("crs_solve_triut:WrongSizeOfInputArg",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs == 2) {
        if (nlhs > 1)
            mexErrMsgIdAndTxt("crs_solve_triut:TooManyOutputArguments",
                "Too many output arguments for entry-point crs_solve_triut.\n");
        /* Call the API function. */
        __crs_solve_triut_api(plhs, prhs);
    }
    else
        mexErrMsgIdAndTxt("crs_solve_triut:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point crs_solve_triut.");
}
