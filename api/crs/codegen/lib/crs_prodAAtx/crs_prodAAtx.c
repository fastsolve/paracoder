  ...
  A_ncols, const emxArray_real_T *x, emxArray_real_T *b)
{
  ...
  i5 = b->size[0];
  crs_prodAtx_kernel(A_row_ptr, A_col_ind, A_val, x, x->size[0], b, i5, A_nrows,
...
#pragma omp master
  ...
  emxInit_real_T(&Atx, 2);
  i7 = Atx->size[0] * Atx->size[1];
  Atx->size[0] = A_ncols;
  Atx->size[1] = x->size[1];
  emxEnsureCapacity((emxArray__common *)Atx, i7, (int)sizeof(double));
  loop_ub = A_ncols * x->size[1];
