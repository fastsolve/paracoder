...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if ( nlhs>1)
        mexErrMsgIdAndTxt("opaque_ptr:WrongNumberOfOutputs",
                "Function opaque_ptr requires one output argument.");
    if ( nrhs==0) {
        mexCallMATLAB( 1, &output, nrhs, (mxArray **)prhs, "opaque_ptr_type");
        plhs[0] = output;
        return;
    }
    if ( nrhs>3)
