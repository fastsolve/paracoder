...
for i=1:ncarg
    ...
    if strncmp(vars(i).type, 'emxArray_', 9) && ...
        ...
        vars(i).isemx = true;
        vars(i).size = determine_type_size(htmlfile, [prefix vars(i).name], 0);
    ...
    if ~isempty(toks{1}{6})
        ...
        vars(i).size = determine_type_size(htmlfile, [prefix vars(i).name], totallen);
        vars(i).modifier = '*';
    ...
    if isempty(vars(i).size)
        vars(i).size = determine_type_size(htmlfile, [prefix vars(i).name], 1);
    end
end
...
function [sz,basetype] = determine_type_size(htmlfile, varname, totalsize)
