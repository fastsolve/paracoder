function [vars, ret, nlhs, nrhs, structDefs, SDindex, pruned_args] = ...
    parse_cgfiles(funcname, altfunc, prototype, args, ...
    inports, outports, cpath, structDefs)
...
[vars, structDefs, SDindex] = parse_var_decl(carglist, ...
    typedecl, hfile, prototype, args, inports, outports, structDefs, '', '');
...
function [vars, structDefs, SDindex] = parse_var_decl(carglist, typedecl, ...
    hfile, prototype, args, inports, outports, structDefs, mprefix, cprefix)
...
for i=1:ncarg
    ...
    if ~isempty(prototype) && isprop(prototype.Arguments(i).Type, 'BaseType') && ...
        ...
        vars(i).mname = '';
    elseif argin_index <= length(inports)
        if ~isempty(args) && isobject(args{argin_index}) && isprop(args{argin_index}, 'SizeVector')
            arg = args{argin_index};
        else
            arg = [];
        end

        size_info = [];
        for j = argin_index:length(inports)
            if isprop(inports(j), 'Implementation') && isempty(inports(j).Implementation)
                continue;
            end

            size_info = inports(j);
            if isprop(size_info, 'GraphicalName')
                vars(i).mname = size_info.GraphicalName;
                if isprop(size_info.Implementation, 'Identifier') || ...
                    strcmp(vars(i).cname(end-4:end), '_size')
                    argin_index = j + 1;
                else
                    assert(strcmp(vars(i).cname(end-4:end), '_data'));
                    argin_index = j;
                end
            else
                vars(i).mname = size_info.Identifier;
                argin_index = j + 1;
            end
            assert(contains(vars(i).cname, vars(i).mname));
            break
        end
    else
        arg = [];
        size_info = [];
        for j = argout_index:length(outports)
            if isempty(outports(j).Implementation)
                continue
            end
            % If an output argument is also an input, it is listed once
            is_input = false;
            for k = 1:length(inports)
                if strcmp(outports(j).GraphicalName, inports(k).GraphicalName)
                    is_input = true;
                    break
                end
            end
            ...
            if isprop(size_info.Implementation, 'Identifier') || ...
                ...
                assert(strcmp(vars(i).cname(end-4:end), '_data'));
                % If variable is (_data, _size) pair, use mname twice
                argout_index = j;
            end
            break
        end
    ...
    if strncmp(vars(i).type, 'emxArray_', 9)
        ...
        [vars(i).size, vars(i).vardim, vars(i).iscomplex] = ...
            determine_type_size(arg, size_info, 0);
    ...
    if ~isempty(toks{1}{6})
        ...
        if i > 1 && isprop(size_info, 'GraphicalName') && ...
            ...
            [vars(i).size, vars(i).vardim, vars(i).iscomplex] = ...
                determine_type_size(arg, size_info, totallen);
        end
    ...
    if isempty(vars(i).size)
        [vars(i).size, vars(i).vardim, vars(i).iscomplex] = ...
            determine_type_size(arg, size_info);
    ...
    if determine_subfields
        ...
        if isprop(arg, 'Fields')
            subfields = struct2cell(arg.Fields);
        else
            subfields = [];
        end
        ...
        [vars(i).subfields, structDefs] = parse_var_decl(sublist{1}, ...
            typedecl, hfile, [], subfields, Elems, [], ...
    ...
    if has_pruned
        ...
        for i=1:length(outports)
            if isempty(outports(i).Implementation)
                ...
                npruned = npruned + 1;
                [pruned_args(npruned).size, ~, pruned_args(npruned).iscomplex, ...
                    pruned_args(npruned).type] = determine_type_size([], outports(i));
            end
        end
    end
end
...
function [sz, vardim, iscomplex, basetype] = determine_type_size(arg, size_info, totalsize)
