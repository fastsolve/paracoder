function writeMexFile(funcname, mpath, cpath, m2c_opts, iscuda)
...
if iscuda
    altapis = filterOutCudaGlobal(funcname, altapis, cpath);
    if isempty(altapis)
        error('m2c:InvalidCudaCode', ...
            'Entry function was not found for CUDA code.');
    end
end
...
for i=1:length(altapis)
    % Parse input arguments from C files.
    [vars, ret, nlhs, nrhs, SDindex, pruned_vars] = parse_cgfiles(funcname, altapis{i}, mpath, cpath);
    if ~isempty(SDindex)
        fprintf(2, ['m2c Info: Codegen generated a StackData object "' ...
            vars(SDindex).cname '" of type "' vars(SDindex).ctype '". ' ...
    ...
    filestr = sprintf('%s', filestr, printApiFunction(funcname, altapis{i}, ...
        nlhs, nrhs, vars, ret, SDindex, pruned_vars, m2c_opts.timing, iscuda));
...
if iscuda && isequal(m2c_opts.suf, 'c')
    filestr = sprintf('%s\n', ...
        '/*', ...
        [' * Auxiliary code for mexFunction of ' funcname], ' *', ...
        ' * to support compilation using C++ compiler.', ...
        ' *', ' */', '', ...
        '/* Include the C file */', ...
        ['#include "' funcname '_mex.c"']);
    outCppFile = [cpath funcname '_mex.cpp'];
    writeFile(outCppFile, filestr);
end
    ...
    elseif var.isemx
        if all(isfinite(var.size)) && any(var.vardim)
            ...
            str = sprintf('%s\n', str, ...
                ...
                var.cname ref 'data, ' getMxClassID(var.basetype) ', ' ...
                num2str(length(sz)) ', ' var.cname ref 'size);']);
...
function altapis = filterOutCudaGlobal(funcname, altapis, cpath)
...
while i<=length(altapis)
    ...
    % If function is global or is not in the header file, remove it
    if ~isempty(regexp(hfile, ['__global__\s+extern\s+(unsigned\s+)?\w+\s+' ...
            altfunc '\s*\([^)]*\)'], 'match', 'once')) || ...
            isempty(regexp(hfile, ['\s+extern\s+(unsigned\s+)?\w+\s+' ...
            altfunc '\s*\([^)]*\)'], 'match', 'once'))
        altapis(i) = [];
    else
        i = i + 1;
