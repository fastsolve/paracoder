...
[marshallin_substr, sub_mx_level_in] = marshallin(altname, vars_ret, nlhs, nrhs, SDindex, parmode);
[marshallout_substr, sub_mx_level_out] = marshallout(vars_ret, nlhs, pruned_vars, parmode);
...
str = sprintf('%s\n', str, ...
    marshallout_substr, '', var_free(vars_ret, parmode), '}');
...
function [str, sub_mx_leve] = marshallin(funcname, vars, nlhs, nrhs, SDindex, parmode)
...
for j=1:nrhs
    ...
    if ~isempty(var.subfields)
        ...
        if ~isequal(parmode, 'cuda-kernel') || isempty(var.modifier)
...
for j=1:nlhs
    ...
    assert(~isequal(parmode, 'cuda-kernel') || prod(var.size)==1);
...
if ~isempty(SDindex)
    assert(~isequal(parmode, 'cuda-kernel'));
    var = vars(SDindex);
...
function [str,sub_mx_level] = marshallout(vars, nlhs, pruned_vars, parmode)
...
for j=1:nlhs
    ...
    elseif ~isempty(var.subfields)
        ...
        sub_mx_level = max(sub_mx_level, sub_mx_level1);
    elseif ~isempty(var.iindex) && var.isemx && isequal(parmode, 'cuda-kernel')
        str = sprintf('%s\n', str, ...
...
function str = var_free(vars, parmode)
