...
elseif isoctave && contains(getenv('OCTAVE_HOME'), 'conda') && ...
    ...
    CC = ['CC=''''' getenv('OCTAVE_HOME'), '/bin/x86_64-conda-linux-gnu-cc' ''''' CXX=''''' getenv('OCTAVE_HOME'), '/bin/x86_64-conda-linux-gnu-c++' ''''''];
end
...
if m2c_opts.genMatlab && ~m2c_opts.quiet
    filestr = sprintf('%s\n', filestr, ...
        '    disp(build_cmd);', ...
        ...
        '    end', ...
        'else', ...
        ['    fprintf(''' funcname '.' mexext__ ' is up to date.\n'');'], ...
        'end');
elseif m2c_opts.genMatlab
    filestr = sprintf('%s\n', filestr, ...
        '    if exist(''OCTAVE_VERSION'', ''builtin'')', ...
        '        status = unix(build_cmd, ''-echo'');', ...
        '        if status; error(''mex failed''); end', ...
