...
for k=1:length(var.subfields)
    ...
    elseif var.subfields(k).isemx
        if ~isempty(var.iindex)
            substr = sprintf( ['%s\n%s    if (%s%s.%s->canFreeData) mxSetFieldByNumber( (mxArray*)(%s), %s, %d, ' ...
                'move_emxArray_to_mxArray((emxArray__common*)%s%s.%s, %s));'], ...
                substr, indent, [prefix var.name], sub, sf, mx, index, k-1, ...
                [prefix var.name], sub, sf, getMxClassID( var.subfields(k).basetype));
        else
