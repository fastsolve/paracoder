function lib2mex(funcname, ldflags, opflags, args, ldlibs)
...
% Write out a build script
writeExeScript(['ld_' funcname '.m'], opts, ldlibs);
% Write M-file wrapper function
writeExeMWrapper(funcname)
...
write_README(outdir, funcname);
    ...
    function writeMexScript(outMfile, opts, libs)
        ...
        fprintf(fid, '%s\n', ...
            ...
            ' ''' funcname '_mex.' suf ' '' output '' ' mexdir funcname ' '' '  opts mpildflag ompldflag libs '];'], ...
            '    fprintf(''Entering %s\n'', pwd);', ...
            '    disp(cmd);', ...
            '    eval(cmd);', ...
    ...
    function writeExeScript(outMfile, opts, libs)        
        if ~isequal(outdir, '.')
            outMfile = [outdir '/' outMfile];
        end
        clear(outMfile);
        ...
        fprintf(fid, '%s\n', ...
            ...
            ['    cmd = [''cc ' ldflags '' '' opflags mpicflag ompcflag ' -I"'' incdir ''" -I"'' m2cdir ''include" -I. '...
             funcname '_mex.' suf ' ' funcname '_exe.' suf ' -o ' exedir funcname '.exe '' '  opts libs mpildflag ompldflag ' matlibs];'], ...
            '    fprintf(''Entering %s\n'', pwd);', ...
            '    disp(cmd);', ...
            '    unix(cmd);', ...
...
for i=1:length(altapis)
    ...
    str = sprintf('\n%s', str, ...
        ...
        ['            m2cErrMsgIdAndTxt("' funcname ':TooManyOutputArguments",' ...
...
str = sprintf('\n%s', ...
    str, '    else', ...
    ['        m2cErrMsgIdAndTxt("' altapis{1} ':WrongNumberOfInputs","' ...
...
function writeExeMWrapper(funcname)
...
for j=1:nrhs
    ...
    if ~isempty(var.subfields)
        ...
        str = sprintf(['%s\n    if (mxGetNumberOfElements(%s) && mxGetClassID(%s) != %s)\n', ...
            '        m2cErrMsgIdAndTxt("%s:WrongInputType",\n',...
        ...
        elseif prod(var.size)~=1
            ...
            str = sprintf(['%s\n    if (mxGetNumberOfElements(%s) != %d)\n', ...
                '        m2cErrMsgIdAndTxt("%s:WrongSizeOfInputArg",\n',...
...
str = sprintf(['%s\n%s    if (!mxIsStruct(%s))\n', ...
    '%s        m2cErrMsgIdAndTxt("%s:WrongInputType",\n',...
...
str = sprintf(['%s\n%s    if (mxGetNumberOfFields(%s)!=%d)\n', ...
    '%s        m2cErrMsgIdAndTxt("%s:InputStructWrongFields",\n',...
...
if prod(var.size)>=1
    str = sprintf(['%s\n%s    if (mxGetNumberOfElements(%s) != %d)\n', ...
        '%s        m2cErrMsgIdAndTxt("%s:WrongSizeOfInputArg",\n',...
...
for k=1:length(var.subfields)
    ...
    substr = sprintf(['%s\n%s    if (%s==NULL)\n', ...
        '%s        m2cErrMsgIdAndTxt("%s:WrongInputStruct",\n',...
